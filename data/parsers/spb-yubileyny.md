...
- Черновик job.config выше финализирован при реализации адаптера (`YubileynyAfishaParser`): добавлен `prices_already_minor: true` (адаптер сам конвертирует «Цена:800 р.» в минорные единицы через `parse_price_to_minor`, как остальные HTML-адаптеры этого репозитория).
