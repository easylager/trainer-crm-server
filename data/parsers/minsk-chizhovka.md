...
4. Kind filter: ячейки МА/БА → `public_skate`, `session_label` = «Малая арена»|«Большая арена» (расшифровка кода МА/БА — клиенту в приложении сырой код с сайта не показываем). Drop «билеты проданы» без времени. ОХМ/школа/аренда льда на этой таблице нет. Если БА и МА в одно время — **два** слота (два льда), не merge.
...
`data/fixtures/minsk-chizhovka/` — `schedule.html`, `prices.html` + `expected.json` (19 слотов, колоночный разбор).
