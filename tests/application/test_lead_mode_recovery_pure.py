...
class TestRecoverySignalsLine:
    ...
    def test_both_views_and_clicks(self):
        ...
        line = _render_recovery_signals_line(views=12, clicks=3, favorites=0)
        assert "12" in line
        assert "3" in line
    ...
    def test_views_only(self):
        ...
        line = _render_recovery_signals_line(views=7, clicks=0, favorites=0)
        assert "7" in line
    ...
    def test_no_demand_returns_empty(self):
        ...
        assert _render_recovery_signals_line(views=0, clicks=0, favorites=0) == ""
    ...
    def test_clicks_without_views_falls_back_to_empty(self):
        ...
        assert _render_recovery_signals_line(views=0, clicks=2, favorites=0) == ""

    def test_favorites_only(self):
        from src.bot.notification_loops import _render_recovery_signals_line

        line = _render_recovery_signals_line(views=0, clicks=0, favorites=4)
        assert "4" in line
        assert "избранное" in line.lower()

    def test_both_plus_favorites_appends_second_sentence(self):
        from src.bot.notification_loops import _render_recovery_signals_line

        line = _render_recovery_signals_line(views=3, clicks=1, favorites=2)
