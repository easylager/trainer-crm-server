...
Максимум **3 кодящих агента** одновременно + сколько угодно **SPEC** (только `data/parsers/` и фикстуры).
...
SPEC-агент код не пишет: `/research` + спека в `data/parsers/`. Цепочка `/execute` ему не нужна, пока нет TASK на код адаптера.
...
|---|---|---|---|---|
| **SPEC** | Реверс катка → спека extract | живые docs | `data/parsers/*.md`, `data/fixtures/<arena>/` | `src/`, миграции, UI |
| **CONTENT** | Досье карточки (не слоты) | 073, 063 | `data/arena-cards/` | `src/`, парсеры, Google-картинки |
