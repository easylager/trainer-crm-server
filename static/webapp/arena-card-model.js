...
(function (root, factory) {
  ...
  function practiceContacts(card) {
    ...
    var socials = [];
    var urls =
      card.social_urls && typeof card.social_urls === "object"
        ? card.social_urls
        : {};
    var i;
    for (i = 0; i < SOCIAL_KEYS.length; i++) {
      ...
      if (href)
        socials.push({ key: key, label: SOCIAL_KEYS[i][1], href: href });
    }
    return {
  ...
  function formatMinor(minor, currency) {
    ...
    var major = n / 100;
    var text = Number.isInteger(major)
      ? String(major)
      : major.toFixed(2).replace(/\.00$/, "");
    if (currency && ruText && typeof ruText.formatPrice === "function") {
      return { amount: text, withCurrency: ruText.formatPrice(text, currency) };
    }
  ...
  function buildRibbonForDay(opts) {
    ...
    for (i = 0; i < groups.length; i++) {
      ...
      for (r = 0; r < rules.length; r++) {
        ...
        var spotsLabel =
          ruText && typeof ruText.countRu === "function"
            ? ruText.countRu(spots, "место", "места", "мест")
            : spots + " места";
        var metaG =
          spots != null && spots > 0
            ? "идёт набор · " + spotsLabel
            : "идёт набор";
  ...
  function sessionTimeRange(sessions) {
    ...
    return (
      "с " +
      times[0] +
      (times.length > 1 ? " до " + times[times.length - 1] : "")
    );
  }
  ...
  function buildWeekSummaries(opts) {
    ...
    while (cursor.getTime() <= end.getTime()) {
      ...
      if (!iceN && !lessonN) {
        ...
        var iceLabel =
          ruText && typeof ruText.countRu === "function"
            ? ruText.countRu(iceN, "сеанс", "сеанса", "сеансов")
            : iceN + " сеансов";
        var lessonLabel =
          ruText && typeof ruText.countRu === "function"
            ? ruText.countRu(lessonN, "занятие", "занятия", "занятий")
            : lessonN + " занятий";
        var titleParts = [];
        if (iceN) titleParts.push(iceLabel);
        if (lessonN) titleParts.push(lessonLabel);
        out.push({
          localDate: key,
          weekday: WEEKDAYS_SHORT[cursor.getDay()],
          empty: false,
  ...
  function buildBookingHref(opts) {
    ...
    if (opts.trainerId != null)
      parts.push("trainer_id=" + encodeURIComponent(String(opts.trainerId)));
    if (opts.arenaId != null)
      parts.push("arena_id=" + encodeURIComponent(String(opts.arenaId)));
    if (opts.action)
      parts.push("action=" + encodeURIComponent(String(opts.action)));
