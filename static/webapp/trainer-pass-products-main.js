(function () {
  ...
  function trainerClientCardUrl(clientId) {
    var path =
      (window.location.pathname || "").replace(/[^/]+$/, "") || "/webapp/";
    return (
      path + "trainer-clients?client_id=" + encodeURIComponent(String(clientId))
    );
  }
  ...
  function syncPassIssueBackLabels() {
    ...
    if (backBtn)
      backBtn.textContent = returnCid ? "К карточке клиента" : "К списку";
  ...
  function showScreen(id) {
    ...
    if (backBtn) {
      ...
      backBtn.onclick = needsAppBack
        ? function () {
            if (id === "screenPassDetail") leavePassDetailScreen();
            else if (id === "screenPassIssue") leavePassIssueScreen();
            else if (id === "screenForm") {
              showScreen("screenList");
              setTab("passes");
              loadList();
            } else if (id === "screenCertForm") {
              showScreen("screenList");
              setTab("certs");
              loadCertList();
            } else if (id === "screenCertIssue") {
              showScreen("screenList");
              setTab("certs");
            } else if (id === "screenPassWelcomeLink") {
              showScreen("screenList");
              setTab("passes");
            }
          }
  ...
  function passDetailLoadingSkeletonHtml() {
    return (
      '<div class="pp-pass-detail-skeleton" aria-hidden="true">' +
  ...
  function fetchPassIssuePrerequisites() {
    var itemsP =
      state.items.length > 0
        ? Promise.resolve()
        : fetch(apiUrl("/trainer/pass-products") + initDataParam(), {
            headers: headers(),
          })
            .then(function (r) {
              return r.json();
    ...
    var clientsP =
      state.clients.length > 0
        ? Promise.resolve()
        : fetch(apiUrl("/trainer/clients") + initDataParam(), {
            headers: headers(),
          })
            .then(function (r) {
              return r.json();
  ...
  function runPassIssueDeepLink(deepLink) {
    ...
    if (deepLink.passProductId)
      state.prefillPassProductIdForPassIssue = deepLink.passProductId;
    setTab("passes");
    showScreen("screenPassIssue");
    setPassIssueBootLoading(true);
    fetchPassIssuePrerequisites()
      .then(function () {
        var activePasses = (state.items || []).filter(function (p) {
          return p.is_active;
        });
        if (!activePasses.length) {
          showPassIssueBootError(
            "Нет активных абонементов",
            "Сначала добавьте абонемент в каталоге.",
          );
          return;
        }
        if (!(state.clients || []).length) {
          showPassIssueBootError(
            "Нет клиентов",
            "Клиенты появятся после записей на занятия.",
          );
          return;
        }
        if (
          !(state.clients || []).some(function (c) {
            return c.id === deepLink.clientId;
          })
        ) {
          showPassIssueBootError(
            "Клиент не найден",
            "Обновите список клиентов и попробуйте снова.",
          );
          return;
        }
      ...
      .catch(function () {
        showPassIssueBootError(
          ...
          "Откройте мини-приложение из бота и попробуйте снова.",
        );
      });
  }
  function setTab(tab) {
  ...
  function renderIssuedPagination() {
    ...
    var html = "";
    html +=
      '<button type="button" class="pp-issued-page-btn" id="issuedPagePrev"' +
      (state.issuedPage <= 0 ? " disabled" : "") +
      ">◀ Назад</button>";
    html +=
      "<span>" +
      from +
      ...
      "</span>";
    html +=
      '<button type="button" class="pp-issued-page-btn" id="issuedPageNext"' +
      (!state.issuedHasMore ? " disabled" : "") +
      ">Вперёд ▶</button>";
  ...
  function issuedStatusBucket(it) {
    if (it.status_bucket === "active" || it.status_bucket === "closed")
      return it.status_bucket;
    if (it.kind === "pass") return it.status === "active" ? "active" : "closed";
    if (it.kind === "certificate") {
      var st = it.status || "";
      return st === "active" || st === "issued" || st === "activated"
        ? "active"
        : "closed";
    }
    return "closed";
  ...
  function renderIssuedSummary() {
    ...
    return (
      '<div class="pp-issued-summary" role="note">' +
  ...
  function renderPassDetailMetaRows(p) {
    ...
    if (p.client_phone)
      rows.push({ label: "Телефон", value: p.client_phone, isPhone: true });
    rows.push({ label: "Услуги", value: p.service_scope || "На все услуги" });
    if (p.issued_at)
      rows.push({ label: "Выдан", value: formatIssuedDate(p.issued_at) });
    if (p.expires_at)
      rows.push({
        label: "Срок",
        value: "до " + formatIssuedDate(p.expires_at),
      });
    var html = "";
    rows.forEach(function (row) {
      ...
      html +=
        ...
        (row.isPhone ? passDetailPhoneHtml(row.value) : escapeHtml(row.value)) +
        "</div>";
      html += "</div></div>";
    });
    host.innerHTML = html;
    if (
      window.CrmPhoneField &&
      typeof window.CrmPhoneField.wirePhoneCallButtons === "function"
    ) {
  ...
  function openPassDetail(passInstanceId, opts) {
    ...
    document.getElementById("passDetailMetaRows").innerHTML =
      passDetailLoadingSkeletonHtml();
    ...
    fetch(
      apiFetchUrl(
        "/trainer/pass-instances/" + encodeURIComponent(String(passInstanceId)),
      ),
      { headers: headers() },
    )
      .then(function (r) {
      ...
      .catch(function (err) {
        document.getElementById("passDetailRedeemableList").innerHTML =
          ...
          escapeHtml(err.message || "Ошибка загрузки") +
          "</p></div>";
      });
  }
  ...
  function renderPassDetailScreen(opts) {
    ...
    document.getElementById("passDetailClient").textContent =
      p.client_name || "Клиент";
    var statusEl = document.getElementById("passDetailStatus");
    if (statusEl) {
      ...
      statusEl.className =
        "pp-pass-detail-status " +
        (st === "active"
          ? "pp-pass-detail-status--live"
          : "pp-pass-detail-status--closed");
    }
    var rem =
      typeof p.sessions_remaining === "number" ? p.sessions_remaining : 0;
    ...
    if (!p.can_redeem) {
      ...
      if (!state.passDetailRedeemable.length) {
        ...
        state.passDetailRedeemable.forEach(function (b) {
          html +=
            '<button type="button" class="pp-redeem-booking-row" data-booking-id="' +
            b.booking_id +
            '">';
          html +=
            '<span class="pp-redeem-booking-row__icon" aria-hidden="true">🏋</span>';
          html += '<span class="pp-redeem-booking-row__main">';
          html +=
            '<div class="pp-redeem-booking-row__date">' +
            escapeHtml(formatBookingSlotLabel(b)) +
            "</div>";
          html +=
            '<div class="pp-redeem-booking-row__meta">' +
        ...
        redeemList
          .querySelectorAll(".pp-redeem-booking-row")
          .forEach(function (btn) {
            btn.onclick = function () {
              var bid = parseInt(btn.dataset.bookingId, 10);
              if (!bid || !state.passDetailId || btn.disabled) return;
              var booking =
                state.passDetailRedeemable.find(function (x) {
                  return x.booking_id === bid;
                }) || {};
              ...
              showAppConfirm(
                ...
                fetch(
                  apiFetchUrl(
                    "/trainer/pass-instances/" +
                      encodeURIComponent(String(state.passDetailId)) +
                      "/redeem",
                  ),
                  {
                    method: "POST",
                  ...
                  .then(function (o) {
                    ...
                    state.passDetailRedemptions = o.data.redemptions || [];
                    state.passDetailRedeemable =
                      o.data.redeemable_bookings || [];
                    ...
                    if (
                      ...
                      window.Telegram.WebApp.showPopup({
                        message:
                          "Занятие списано · осталось " +
                          (state.passDetail.sessions_remaining || 0) +
                          " из " +
                          (state.passDetail.sessions_total || 0),
                        buttons: [{ type: "ok" }],
                      });
                    }
                  })
    ...
    if (!state.passDetailRedemptions.length) {
      ...
      state.passDetailRedemptions.forEach(function (r) {
        ...
        histHtml +=
          '<span class="pp-redemption-row__dot" aria-hidden="true"></span>';
        histHtml += '<div class="pp-redemption-row__text">';
        histHtml +=
          '<div class="pp-redemption-row__date">' +
          escapeHtml(formatBookingSlotLabel(r)) +
          "</div>";
        histHtml +=
          '<div class="pp-redemption-row__service">' +
          escapeHtml(r.service_name || "Занятие") +
          "</div>";
        histHtml += "</div></div>";
      });
  ...
  function leavePassDetailScreen() {
    if (state.passDetailReturnClientId) {
      window.location.href = trainerClientCardUrl(
        state.passDetailReturnClientId,
      );
      return;
    }
    showScreen("screenList");
  ...
  function renderIssuedList() {
    ...
    if (!state.issuedItems.length) {
      ...
      wrap.innerHTML =
        '<div class="pp-state pp-state--empty"><div class="pp-state-icon" aria-hidden="true">📋</div><p class="pp-state-title">Пока ничего не выдано</p><p class="pp-state-text">' +
    ...
    state.issuedItems.forEach(function (it) {
      ...
      var lifeCls =
        bucket === "active" ? "pp-issued-card--live" : "pp-issued-card--closed";
      var statusCls =
        bucket === "active"
          ? "pp-issued-status--live"
          : "pp-issued-status--closed";
      var clickable =
        it.kind === "pass" &&
        bucket === "active" &&
        (it.sessions_remaining || 0) > 0;
      var cardTag = clickable ? "button" : "div";
      var cardExtra = clickable ? " pp-issued-card--clickable" : "";
      var cardAttrs = clickable
        ? ' type="button" class="pp-issued-card ' +
          kindCls +
          " " +
          lifeCls +
          cardExtra +
      ...
      html +=
        '<span class="pp-issued-kind">' +
        escapeHtml(issuedKindLabel(it.kind)) +
        "</span>";
      html +=
        '<span class="pp-issued-status ' +
        statusCls +
        '">' +
        escapeHtml(it.status_label_ru || it.status || "—") +
        "</span>";
      html += "</div>";
      html +=
        '<div class="pp-issued-date">' +
        escapeHtml(formatIssuedDate(it.issued_at)) +
        "</div>";
      html +=
        '<div class="pp-issued-title">' +
        escapeHtml(it.product_name || "—") +
        "</div>";
      html +=
        '<div class="pp-issued-meta">Клиент: ' +
        escapeHtml(it.client_label_ru || "—") +
        "</div>";
      if (it.client_phone) {
        html +=
          '<div class="pp-issued-meta">' +
          escapeHtml(it.client_phone) +
          "</div>";
      }
      if (it.kind === "pass") {
        html +=
          '<div class="pp-issued-balance"><strong>' +
          (it.sessions_remaining != null ? it.sessions_remaining : 0) +
          "</strong> из " +
          (it.sessions_total != null ? it.sessions_total : 0) +
          " занятий</div>";
        if (it.service_scope) {
          html +=
            '<div class="pp-issued-meta">Услуги: ' +
            escapeHtml(it.service_scope) +
            "</div>";
        } else {
        ...
        if (clickable) {
          html +=
            '<div class="pp-issued-redeem-cta">Списать прошедшее занятие</div>';
        }
      } else {
        if (
          it.recipient_name &&
          it.recipient_name !== (it.client_label_ru || "")
        ) {
          html +=
            '<div class="pp-issued-meta">Получатель: ' +
            escapeHtml(it.recipient_name) +
            "</div>";
        }
        if (it.amount_remaining_cents != null) {
          html +=
            '<div class="pp-issued-balance">Остаток: <strong>' +
            escapeHtml(formatPricePlain(it.amount_remaining_cents)) +
            "</strong></div>";
        } else if (it.amount_cents != null) {
          html +=
            '<div class="pp-issued-balance">Номинал: <strong>' +
            escapeHtml(formatPricePlain(it.amount_cents)) +
            "</strong></div>";
        }
        if (it.code) {
          html +=
            '<div class="pp-issued-meta pp-issued-code">Код: ' +
            escapeHtml(it.code) +
            "</div>";
        }
      }
      if (it.expires_at) {
        html +=
          '<div class="pp-issued-meta">Срок: до ' +
          escapeHtml(formatIssuedDate(it.expires_at)) +
          "</div>";
      }
      html += "</" + cardTag + ">";
    });
  ...
  function loadIssuedList() {
    if (
      ...
      window.TrainerMiniAppGate.shouldBlockFeatureFetch()
    )
      return;
    var host = document.getElementById("issuedListContent");
    if (!host) return;
    host.innerHTML =
      '<div class="pp-state pp-state--loading"><div class="pp-state-icon" aria-hidden="true">⏳</div><p class="pp-state-title">Загрузка</p><div class="pp-loading-dots" aria-hidden="true"><span></span><span></span><span></span></div></div>';
    var pagEl = document.getElementById("issuedPagination");
    if (pagEl) {
      pagEl.hidden = true;
      pagEl.innerHTML = "";
    ...
    fetch(apiFetchUrl(path), { headers: headers() })
      ...
      .then(function (o) {
        if (!o.ok) {
          var detail = o.data && o.data.detail;
          var msg =
            typeof detail === "string" ? detail : "Не удалось загрузить список";
          throw new Error(msg);
        }
        state.issuedItems = o.data && o.data.items ? o.data.items : [];
        state.issuedTotal =
          o.data && typeof o.data.total === "number"
            ? o.data.total
            : state.issuedItems.length;
        state.issuedActiveCount =
          o.data && typeof o.data.active_count === "number"
            ? o.data.active_count
            : 0;
      ...
      .catch(function () {
        host.innerHTML =
          '<div class="pp-state pp-state--error"><div class="pp-state-icon" aria-hidden="true">⚠️</div><p class="pp-state-title">Не удалось загрузить</p><p class="pp-state-text">Проверьте соединение и попробуйте снова.</p></div>';
        if (pagEl) {
          pagEl.hidden = true;
          pagEl.innerHTML = "";
        }
  ...
  function formatSessions(count) {
    return ruText.countRu(count, "занятие", "занятия", "занятий");
  }
  ...
  function renderList() {
    ...
    if (hintEl)
      hintEl.style.display = state.items.length > 0 ? "block" : "none";
    if (state.items.length === 0) {
      wrap.innerHTML =
        '<div class="pp-state pp-state--empty"><div class="pp-state-icon" aria-hidden="true">📦</div><p class="pp-state-title">Пока нет абонементов</p><p class="pp-state-text">Добавьте первый — клиенты смогут покупать их у вас через платформу.</p></div>';
      return;
    }
    var html = "";
    state.items.forEach(function (p) {
      var serviceLabel =
        p.service_name && p.service_name.trim()
          ? p.service_name.trim()
          : "любая услуга";
      var scopeParts = [serviceLabel];
      if (p.tiers_label && p.tiers_label.trim())
        scopeParts.push(p.tiers_label.trim());
      var meta =
        formatSessions(p.sessions_total) +
        " · " +
        formatPrice(p.price_cents) +
        " · " +
        scopeParts.join(", тарифы: ");
      ...
      if (p.scope_valid === false && p.scope_warning) {
        html +=
          '<div class="product-card-scope-warn">' +
          escapeHtml(p.scope_warning) +
          "</div>";
      }
      html += "</div>";
    ...
    wrap.querySelectorAll(".product-card").forEach(function (btn) {
      btn.onclick = function () {
        ...
        var p = state.items.find(function (x) {
          return x.id === id;
        });
        if (p) {
  ...
  function renderPassProductServiceCheckboxes(selectedIds) {
    ...
    if (!(state.services || []).length) {
      host.innerHTML =
        '<div class="cert-issued-empty">Нет услуг — добавьте услуги в профиле тренера.</div>';
      return;
    }
  ...
  function getPassProductAvailableTierKinds(serviceIds) {
    ...
    if (!services.length) {
      return TIER_KINDS.map(function (t) {
        return t.kind;
      });
    }
    ...
    var pool = services.filter(function (s) {
      return (
        serviceIds.indexOf(s.id) >= 0 || serviceIds.indexOf(String(s.id)) >= 0
      );
    });
    ...
    return TIER_KINDS.map(function (t) {
      ...
      return pool.every(function (s) {
        return (s.price_tiers || []).some(function (pt) {
          return pt.tier_kind === kind;
        });
      });
    });
  }
  ...
  function updatePassTiersHint(serviceIds, availableKinds) {
    ...
    if (serviceIds && serviceIds.length) {
      hint.innerHTML =
        "Показаны тарифы, которые настроены для <b>всех</b> выбранных услуг.";
      return;
    }
    hint.innerHTML =
      "Не отмечайте ни одного — абонемент подходит ко <b>всем</b> тарифам. Иначе — только отмеченные тарифы из профиля.";
  }
  ...
  function onPassProductScopeChanged() {
    ...
    selectedTiers = selectedTiers.filter(function (k) {
      return available.indexOf(k) >= 0;
    });
  ...
  function renderPassProductTierCheckboxes(selectedKinds, availableKinds) {
    ...
    var selectedSvc = getPassProductSelectedServiceIds();
    if (!availableKinds)
      availableKinds = getPassProductAvailableTierKinds(selectedSvc);
    var availableSet = {};
    (availableKinds || []).forEach(function (k) {
      availableSet[k] = true;
    });
    var sel = {};
    (selectedKinds || []).forEach(function (k) {
      sel[k] = true;
    });
    host.innerHTML = "";
    if (!availableKinds.length && !(selectedKinds || []).length) {
      host.innerHTML =
        '<div class="cert-issued-empty">Нет доступных тарифов для текущего выбора услуг.</div>';
    ...
    TIER_KINDS.forEach(function (t) {
      ...
      var row = document.createElement("label");
      row.className =
        "pp-svc-row" + (availableSet[t.kind] ? "" : " pp-tier-row--invalid");
      var cb = document.createElement("input");
      cb.type = "checkbox";
      ...
      var span = document.createElement("span");
      span.textContent =
        t.label + (availableSet[t.kind] ? "" : " · не в профиле");
      row.appendChild(span);
      host.appendChild(row);
    });
    updatePassTiersHint(selectedSvc, availableKinds);
  ...
  function repaintPassProductScopeFields(preSelectedSvc, preSelectedTiers) {
    ...
    var tiers = (preSelectedTiers || []).filter(function (k) {
      return available.indexOf(k) >= 0;
    });
  ...
  function openForm(product) {
    ...
    document.getElementById("inputSessions").value = product
      ? String(product.sessions_total)
      : "";
    document.getElementById("inputPrice").value = product
      ? String(Math.round(product.price_cents / 100))
      : "";
    document.getElementById("inputActive").checked = product
      ? product.is_active
      : true;
    updateActiveLabel();
    document.getElementById("groupActive").style.display = product
      ? "block"
      : "none";
    document.getElementById("btnDeleteProduct").style.display = product
      ? "block"
      : "none";
    state.editingId = product ? product.id : null;
    var preSelectedSvc =
      product && Array.isArray(product.service_ids)
        ? product.service_ids.slice()
        : [];
    var preSelectedTiers =
      product && Array.isArray(product.tier_kinds)
        ? product.tier_kinds.slice()
        : [];
    ...
    if (state.services.length === 0) {
      fetch(apiUrl("/trainer/my-services") + initDataParam(), {
        headers: headers(),
      })
        .then(function (r) {
          return r.json();
  ...
  function loadList() {
    ...
    fetch(apiUrl("/trainer/pass-products") + initDataParam(), {
      ...
      .catch(function () {
        document.getElementById("listContent").innerHTML =
          '<div class="pp-state pp-state--error"><div class="pp-state-icon" aria-hidden="true">⚠️</div><p class="pp-state-title">Не удалось загрузить</p><p class="pp-state-text">Откройте мини-приложение из бота и попробуйте снова.</p></div>';
      });
  }
  ...
  function applyPassIssueClientFilter() {
    ...
    if (!q) {
      ...
      state.passIssueFilteredClients = (state.clients || []).filter(
        function (c) {
  ...
  function renderPassIssueClientList() {
    ...
    var list = state.passIssueSelectedClientId
      ? []
      : state.passIssueFilteredClients;
    if (list.length === 0) {
      wrap.innerHTML =
        '<div class="cert-issued-empty">' +
        (state.passIssueFilteredClients.length === 0 &&
        (document.getElementById("passIssueClientSearch").value || "").trim()
          ? "Никого не найдено"
    ...
    list.forEach(function (cl) {
      ...
      var selected =
        state.passIssueSelectedClientId === cl.id ? " selected" : "";
      html +=
        '<button type="button" class="pass-issue-client-card' +
        selected +
        '" data-id="' +
        cl.id +
        '">';
      html +=
        '<span class="pass-issue-client-name">' + escapeHtml(name) + "</span>";
      html +=
        '<span class="pass-issue-client-phone">' +
        escapeHtml(phone) +
        "</span>";
      html += "</button>";
    });
    ...
    wrap.querySelectorAll(".pass-issue-client-card").forEach(function (btn) {
      btn.onclick = function () {
        ...
        document.getElementById("passIssueSelectedName").textContent =
          [cl.first_name, cl.last_name].filter(Boolean).join(" ").trim() ||
          cl.phone ||
          "Клиент #" + cl.id;
        document.getElementById("passIssueSelectedPhone").textContent = (
          cl.phone || ""
        ).trim()
          ? cl.phone || ""
          : "";
        document.getElementById("passIssueSelectedPhone").style.display = (
          cl.phone || ""
        ).trim()
          ? ""
          : "none";
        document.getElementById("passIssueClientSearchGroup").style.display =
          "none";
        document.getElementById("passIssueSelectedGroup").style.display =
          "block";
      };
    });
  }
    ...
    activePasses.forEach(function (p) {
      ...
      var serviceLabel =
        p.service_name && p.service_name.trim()
          ...
          : "любая услуга";
      var scopeStr =
        p.tiers_label && p.tiers_label.trim()
          ? serviceLabel + ", тарифы: " + p.tiers_label.trim()
          : serviceLabel;
      opt.textContent =
        (p.name || "") +
        " · " +
  ...
  document.getElementById("btnPassIssueSuccessBack").onclick =
    leavePassIssueScreen;
  document.getElementById("btnSubmitPassIssue").onclick = function () {
    ...
    var productId = parseInt(
      document.getElementById("passIssueProductSelect").value,
      10,
    );
    if (!clientId) {
  ...
  document.getElementById("certAnyAmount").onchange = function () {
    document.getElementById("certAmountGroup").style.display =
      document.getElementById("certAnyAmount").checked ? "none" : "block";
  };
  ...
  function certProductAmountLine(c) {
    ...
    if (name.indexOf("BYN") !== -1 || name.indexOf("Любая сумма") !== -1)
      return "";
    return amount;
  }
  ...
  function renderCertList() {
    ...
    if (hintEl)
      hintEl.style.display = state.certItems.length > 0 ? "block" : "none";
    if (state.certItems.length === 0) {
      wrap.innerHTML =
        '<div class="pp-state pp-state--empty"><div class="pp-state-icon" aria-hidden="true">🎁</div><p class="pp-state-title">Пока нет сертификатов</p><p class="pp-state-text">Номинал на сумму или «любая сумма» — клиенты увидят это в вашей карточке.</p></div>';
      return;
    }
    var html = "";
    state.certItems.forEach(function (c) {
      ...
      if (amountLine) {
        html +=
          '<div class="cert-product-card__amount">' +
          escapeHtml(amountLine) +
          "</div>";
      }
      if (desc) {
        html +=
          '<div class="cert-product-card__desc">' +
          escapeHtml(desc.length > 120 ? desc.slice(0, 117) + "…" : desc) +
          "</div>";
      }
      html += "</div>";
      html += '<span class="arrow">→</span></button>';
    ...
    wrap.querySelectorAll(".cert-product-card").forEach(function (btn) {
      btn.onclick = function () {
        var id = parseInt(btn.dataset.id, 10);
        var c = state.certItems.find(function (x) {
          return x.id === id;
        });
        if (c) {
  ...
  function loadCertList() {
    ...
    host.innerHTML =
      '<div class="pp-state pp-state--loading"><div class="pp-state-icon" aria-hidden="true">⏳</div><p class="pp-state-title">Загрузка</p><div class="pp-loading-dots" aria-hidden="true"><span></span><span></span><span></span></div></div>';
    fetch(apiUrl("/trainer/certificate-products") + initDataParam(), {
      headers: headers(),
    })
      .then(function (r) {
        return r.json();
      ...
      .catch(function () {
        host.innerHTML =
          '<div class="pp-state pp-state--error"><div class="pp-state-icon" aria-hidden="true">⚠️</div><p class="pp-state-title">Не удалось загрузить</p><p class="pp-state-text">Проверьте соединение и откройте экран из бота.</p></div>';
      });
  }
  function openCertForm(cert) {
    document.getElementById("certFormTitle").textContent = cert
    ...
    document.getElementById("certAmountGroup").style.display = anyAmount
      ? "none"
      : "block";
    document.getElementById("certAmountByn").value =
      cert && cert.amount_cents != null
        ? String(Math.round(cert.amount_cents / 100))
        : "";
    document.getElementById("certActive").checked = cert
      ? cert.is_active
      : true;
    document.getElementById("certActiveGroup").style.display = cert
      ? "block"
      : "none";
    document.getElementById("btnDeleteCert").style.display = cert
      ? "block"
      : "none";
  ...
  function filterCertIssueClients() {
    ...
    if (!q) {
      ...
      var qLower = q.toLowerCase();
      var digits = q.replace(/\D/g, "");
      state.certIssueFilteredClients = (state.clients || []).filter(
        function (c) {
          var name = ((c.first_name || "") + " " + (c.last_name || ""))
            .trim()
            .toLowerCase();
  ...
  function renderCertIssueClientList() {
    ...
    var list = state.certIssueSelectedClientId
      ? []
      : state.certIssueFilteredClients || [];
    if (list.length === 0) {
      var q = (
        document.getElementById("certIssueClientSearch").value || ""
      ).trim();
      wrap.innerHTML =
        ...
        (q
          ? "Никого не найдено"
          : "Введите имя или телефон или оставьте пусто") +
        "</div>";
      return;
    }
    var html = "";
    list.forEach(function (cl) {
      ...
      var phone = (cl.phone || "").trim() || "—";
      var selected =
        state.certIssueSelectedClientId === cl.id ? " selected" : "";
      html +=
        '<button type="button" class="pass-issue-client-card' +
        selected +
        '" data-id="' +
    ...
    wrap.querySelectorAll(".pass-issue-client-card").forEach(function (btn) {
      btn.onclick = function () {
        ...
        document.getElementById("certIssueSelectedName").textContent =
          [cl.first_name, cl.last_name].filter(Boolean).join(" ").trim() ||
          cl.phone ||
          "Клиент #" + cl.id;
        document.getElementById("certIssueSelectedPhone").textContent = (
          cl.phone || ""
        ).trim()
          ? cl.phone || ""
          : "";
        document.getElementById("certIssueSelectedPhone").style.display = (
          cl.phone || ""
        ).trim()
          ? ""
          : "none";
        document.getElementById("certIssueClientSearchGroup").style.display =
          "none";
        document.getElementById("certIssueSelectedGroup").style.display =
          "block";
      };
    });
  }
  function applyCertIssuePrefillFromState() {
    ...
    if (rn && state.prefillCertRecipientName)
      rn.value = state.prefillCertRecipientName;
    if (re && state.prefillCertRecipientEmail)
      re.value = state.prefillCertRecipientEmail;
  ...
  function openCertIssueScreen() {
    ...
    var activeCerts = (state.certItems || []).filter(function (c) {
      return c.is_active;
    });
  ...
  if (btnIssueCertEl) {
    btnIssueCertEl.onclick = function () {
      ...
      var clientsPromise = fetch(apiUrl("/trainer/clients") + initDataParam(), {
        headers: headers(),
      })
        .then(function (r) {
          return r.json();
  ...
  document
    .getElementById("certIssueClientSearch")
    .addEventListener("input", filterCertIssueClients);
  ...
  document.getElementById("btnSubmitCertIssue").onclick = function () {
    ...
    var productId = parseInt(
      document.getElementById("issueProductSelect").value,
      10,
    );
    var recipientName = (
      document.getElementById("issueRecipientName").value || ""
    ...
    var recipientEmail =
      (document.getElementById("issueRecipientEmail").value || "").trim() ||
      null;
    if (!productId) {
      alert("Выберите сертификат (номинал)");
      return;
    }
    if (!recipientName) {
      alert("Укажите имя получателя — оно будет указано в сертификате");
      return;
    }
    var body = {
      certificate_product_id: productId,
      recipient_name: recipientName,
    };
    if (recipientEmail) body.recipient_email = recipientEmail;
    if (state.certIssuePurchasedByName)
      body.purchased_by_name = state.certIssuePurchasedByName;
      ...
      .then(function (o) {
        if (o.ok && o.data.code) {
          ...
          if (o.data.id && o.data.file_url) {
            downloadLink.href =
              apiUrl("/trainer/certificates/" + o.data.id + "/file") +
              initDataParam();
          ...
          if (recipientEmail) {
            ...
            if (o.data.email_sent) {
              emailSentEl.textContent =
                "Сертификат отправлен на " + recipientEmail;
              emailSentEl.style.color = "";
            } else if (o.data.email_pending) {
              emailSentEl.textContent =
                "Письмо будет отправлено в ближайшее время (при сбое — повторная отправка по очереди).";
              emailSentEl.style.color = "var(--tg-theme-hint-color, #666)";
            } else {
              emailSentEl.textContent =
                "Письмо не удалось отправить на " +
                recipientEmail +
                ". Проверьте настройки SMTP в .env и логи сервера.";
              emailSentEl.style.color =
  ...
  function openPassWelcomeLinkScreen() {
    ...
    activePasses.forEach(function (p) {
      ...
      opt.textContent =
        (p.name || "").trim() ||
        p.sessions_total + " занятий · " + formatPricePlain(p.price_cents);
      sel.appendChild(opt);
    });
  ...
  if (btnPassWelcomeLinkEl) {
    btnPassWelcomeLinkEl.onclick = function () {
      ...
      promise.then(openPassWelcomeLinkScreen).catch(function () {
        state.items = [];
        openPassWelcomeLinkScreen();
      });
    };
  }
  ...
  document.getElementById("btnGetPassWelcomeLink").onclick = function () {
    var productId = parseInt(
      document.getElementById("passWelcomeProductSelect").value,
      10,
    );
    if (!productId) {
      alert("Выберите абонемент");
    ...
    var url =
      apiUrl("/trainer/welcome-link/pass") +
    ...
    fetch(url, { headers: headers() })
      ...
      .then(function (o) {
        ...
        if (link) {
          ...
          document.getElementById("passWelcomeLinkResult").style.display =
            "block";
        } else {
      ...
      fetch(apiUrl("/trainer/certificate-products/" + state.editingCertId), {
        ...
        .then(function (o) {
          if (o.ok) {
            ...
            alert(o.data.detail || "Ошибка сохранения");
          }
        })
        .catch(function () {
          alert("Ошибка сети");
      ...
      fetch(apiUrl("/trainer/certificate-products"), {
        ...
        .then(function (o) {
          if (o.ok && o.data.id) {
            ...
            alert(o.data.detail || "Ошибка создания");
          }
        })
        .catch(function () {
          alert("Ошибка сети");
  ...
  document.getElementById("btnDeleteCert").onclick = function () {
    ...
    showAppConfirm("Удалить этот сертификат?", {
      okText: "Удалить",
      cancelText: "Отмена",
    }).then(function (ok) {
      if (!ok) return;
      fetch(apiUrl("/trainer/certificate-products/" + state.editingCertId), {
        ...
        .then(function (o) {
          if (o.ok) {
            ...
            alert(o.data.detail || "Ошибка удаления");
          }
        })
        .catch(function () {
          alert("Ошибка сети");
  ...
  document.getElementById("btnSaveForm").onclick = function () {
    ...
    if (tierKinds.length && availableTiers.length) {
      tierKinds = tierKinds.filter(function (k) {
        return availableTiers.indexOf(k) >= 0;
      });
    }
    if (
      tierKinds.length === 0 &&
      availableTiers.length === 0 &&
      getPassProductSelectedTierKinds().length
    ) {
    ...
    if (state.editingId) {
      var body = {
        ...
        is_active: document.getElementById("inputActive").checked,
  ...
  document.getElementById("btnDeleteProduct").onclick = function () {
    ...
    showAppConfirm(
      "Удалить этот абонемент? Удаление возможно только если по нему не было покупок.",
      ...
      fetch(apiUrl("/trainer/pass-products/" + state.editingId), {
        ...
        .then(function (o) {
          if (o.ok) {
            ...
            alert(
              o.data.detail ||
                "Не удалось удалить. Возможно, по абонементу уже были покупки — тогда деактивируйте его (снимите галочку «Активен»).",
            );
          }
        })
        .catch(function () {
  ...
  (function checkCertIssuePrefillFromRequest() {
    ...
    state.prefillCertRecipientEmail = (
      params.get("recipient_email") || ""
    ).trim();
    state.prefillCertRecipientName = (
    ...
    setTab("certs");
    var certsP = fetch(
      apiUrl("/trainer/certificate-products") + initDataParam(),
      { headers: headers() },
    )
      .then(function (r) {
        return r.json();
    ...
    var clientsP = fetch(apiUrl("/trainer/clients") + initDataParam(), {
      headers: headers(),
    })
      .then(function (r) {
        return r.json();
    ...
    Promise.all([certsP, clientsP]).then(function () {
      var active = (state.certItems || []).filter(function (c) {
        return c.is_active;
      });
