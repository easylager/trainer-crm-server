(function () {
  ...
  function setArenaSelection(ids, names) {
    ...
    if (safeIds.length === 1) {
      ...
      state.arenaName =
        safeIds.length === 0
          ? "Любая"
          : ruText.countRu(safeIds.length, "арена", "арены", "арен");
    }
  ...
  function primaryVenueLabel(t) {
    if (!t) return "";
    if (t.primary_arena_name && String(t.primary_arena_name).trim())
      return String(t.primary_arena_name).trim();
    var ids = t.arena_ids || [];
  ...
  function buildSlotVenueBlockHtml(slot) {
    ...
    var city =
      (slot.arena_city_name && String(slot.arena_city_name).trim()) || "";
    ...
    if (name || addr || city) {
      ...
      parts.push(
        '<div class="booking-venue-slot-title"><strong>Где</strong></div>',
      );
      parts.push(
        '<div class="booking-venue-slot-lines">' + escapeHtml(loc) + "</div>",
      );
    } else {
    ...
    return parts.length
      ? '<div class="booking-venue-slot">' + parts.join("") + "</div>"
      : "";
  }
  ...
  function updateBookingVenueHint() {
    ...
    if (slotHtml) chunks.push(slotHtml);
      ...
      chunks.push(
        '<div class="booking-venue-slot-lines">Площадка: ' +
          escapeHtml(label) +
          "</div>",
      );
    }
    var mismatchNotice =
      ...
      typeof window.BookingClient.formatPlaceMismatchNotice === "function"
        ? window.BookingClient.formatPlaceMismatchNotice(slot)
        : "";
    if (mismatchNotice) {
      chunks.push(
        '<p class="booking-place-mismatch">' +
          escapeHtml(mismatchNotice) +
          "</p>",
      );
    }
  ...
  function getSelectedCatalogBookProfile() {
    ...
    for (var i = 0; i < catalogBookProfiles.length; i++) {
      if (catalogBookProfiles[i].client_id === catalogActingProfileId)
        return catalogBookProfiles[i];
    }
    return null;
  }
  ...
  function updateCatalogBookSubmitLabel() {
    ...
    if (p && p.role === "guardian") {
      var name =
        (p.first_name || "").trim() || catalogBookProfileDisplayName(p);
    ...
    var preferred =
      catalogActingProfileId != null
        ...
        : window.ClientProfileSwitcher &&
          ...
          ? ClientProfileSwitcher.getActiveProfileId()
          : null;
    if (preferred == null && catalogBookProfiles[0])
      preferred = catalogBookProfiles[0].client_id;
  ...
  function loadCatalogBookProfiles() {
    return Promise.resolve()
      .then(function () {
        if (
          window.ClientProfileSwitcher &&
          typeof ClientProfileSwitcher.init === "function"
        ) {
          return ClientProfileSwitcher.init().then(function (st) {
            catalogBookProfiles =
              (st && st.profiles) || ClientProfileSwitcher.getProfiles() || [];
          });
        }
        catalogBookProfiles = [];
      })
    ...
    refreshClientPhoneForBookingForm(function () {
      loadCatalogBookProfiles().then(function () {
        ...
        if (window.BookingClient) {
          window.BookingClient.emitBookingAnalytics("booking_step_viewed", {
            step: "form",
            host: "catalog",
          });
        }
  ...
  function buildTrainerDetailSlotsSkeletonHtml() {
    ...
    for (var i = 0; i < 3; i++) {
      rows +=
        '<div class="catalog-trainer-detail-skel__slot-row catalog-skel-shimmer"></div>';
    }
    return (
  ...
  function openTrainerDeepLinkSlotPick(
    returnCtx,
    explicitTrainerArenaIdsFromUrl,
    onFail,
  ) {
    loadTrainerById(returnCtx.trainer_id)
      .then(function (t) {
        if (!t) {
          if (onFail) onFail();
          return;
        ...
        sessionReq
          ...
          .then(function () {
            initTrainerSlotsArenaFilter(t, {
              explicitArenaIds: explicitTrainerArenaIdsFromUrl,
            });
          ...
          .then(function (slots) {
            ...
            if (
              state.pendingDeepLinkSlotId &&
              maybeOpenPendingDeepLinkSlotBooking(t, slots)
            )
              return;
            clearCatalogDeepLinkShellClasses();
            renderSlotPickList();
  ...
  function openTrainerDeepLinkTrainerCard(
    returnCtx,
    explicitTrainerArenaIdsFromUrl,
    onFail,
  ) {
  ...
  function catalogParseBookingJsonResponse(text) {
    if (window.BookingClient)
      return window.BookingClient.parseBookingJsonResponse(text);
    try {
  ...
  function selectCatalogBookingService(serviceId) {
    ...
    var tid =
      state.selectedTrainer && state.selectedTrainer.id != null
        ? state.selectedTrainer.id
        : null;
    if (tid != null && state.cityId && state.serviceId != null)
      persistTrainerSelection(tid);
    updateBookingFormServiceAndTiers();
  }
  ...
  function updateBookingFormServiceAndTiers() {
    ...
    var nm =
      effSid != null
        ? catalogServiceDisplayNameForId(effSid)
        : (state.serviceName || "").trim();
    var t = state.selectedTrainer;
    var services = t && t.services ? t.services : [];
    var slot = state.selectedSlot;
    var capRaw =
      slot && slot.capacity != null ? parseInt(slot.capacity, 10) : 1;
    ...
    if (svcBlock) {
      if (!services.length) {
        ...
        if (svcSelect) {
          ...
          if (
            pickSid != null &&
            svcSelect.querySelector('option[value="' + String(pickSid) + '"]')
          ) {
    ...
    tiers.forEach(function (tier) {
      ...
      var priceHtml = escapeHtml(ruText.formatPrice(priceNum, "BYN"));
      lab.appendChild(inp);
      var tierTxt = document.createElement("span");
      tierTxt.className = "tier-radio-text";
      tierTxt.innerHTML =
        escapeHtml(catalogTierLabelRu(tier)) + " — " + priceHtml;
    ...
    var pick =
      preferredId != null
        ? host.querySelector(
            'input[name="catalogPriceTierChoice"][value="' + preferredId + '"]',
          )
        : null;
    if (pick) {
      pick.checked = true;
  ...
  function canBrowserGoBack() {
    try {
      if (
        window.navigation &&
        typeof window.navigation.canGoBack === "function"
      ) {
    ...
    if (sid === "screenBookingForm" || sid === "screenSlotPick") {
      ...
      btn.onclick = function () {
        if (sid === "screenSlotPick" && isHubDirectBookEntry()) {
          if (
            window.BookingClient &&
            typeof window.BookingClient.navigateBookingReturn === "function"
          ) {
  ...
  function setCatalogListLoading(el, count) {
    ...
    if (
      window.ClientShell &&
      typeof window.ClientShell.renderSkeletonList === "function"
    ) {
  ...
  function showScreen(id) {
    ...
    if (id === "screenTrainerDetail") {
      document
        .querySelectorAll("#catalogTabsOnDetail .tab-btn")
    ...
    syncCatalogTrainerStickyCta(
      id === "screenTrainerDetail" && catalogTrainerStickyEligible(),
    );
  }
  ...
  function bindTrainerDetailPhotoWrap() {
    var wrap = document.querySelector(
      "#trainerDetailTop .trainer-detail-photo-wrap",
    );
    if (!wrap) return;
    ...
    if (single) {
      ...
      if (single.complete && single.naturalWidth) {
        single.classList.add("loaded");
        hidePh();
      }
      return;
  ...
  function makeTrainerListParams(offset) {
    ...
    else if (state.arenaIds.length > 1)
      params.arena_ids = state.arenaIds.join(",");
    var f = state.filters;
    if (f.days.length > 0) {
  ...
  function makeFirstPageKey() {
    ...
    var dayPart = f.days.length
      ? f.days
          ...
          .join("-")
      : "";
    var timePart = f.timeSlots.length
      ? f.timeSlots.slice().sort().join("|")
      : "";
  ...
  function collectiveContactLines(brand) {
    ...
    if (phone)
      lines.push(
        '<div class="empty-state-text"><strong>Телефон:</strong> ' +
          phoneCallHtml(phone) +
          "</div>",
      );
    if (tg)
      lines.push(
        '<div class="empty-state-text"><strong>Telegram:</strong> ' +
          escapeHtml(tg) +
          "</div>",
      );
    if (ig)
      lines.push(
        '<div class="empty-state-text"><strong>Instagram:</strong> ' +
          escapeHtml(ig) +
          "</div>",
      );
    if (address)
      lines.push(
        '<div class="empty-state-text"><strong>Адрес:</strong> ' +
          escapeHtml(address) +
          "</div>",
      );
    return lines.join("");
  ...
  function buildStudioRosterEmptyHtml(brand) {
    ...
    return (
      '<div class="empty catalog-collective-roster-empty">' +
      '<p class="empty-state-text"><strong>' +
      name +
      "</strong> скоро откроется — команда набирает тренеров.</p>" +
      (contacts ||
        '<p class="empty-state-text">Следите за обновлениями в наших соцсетях.</p>') +
      "</div>"
    );
  }
  ...
  function syncCollectiveCatalogModeTabs(brand) {
    ...
    if (
      brand &&
      isCenterCatalogBrand(brand) &&
      brand.show_coaches_catalog_tab
    ) {
      row.hidden = false;
      row.innerHTML =
        '<button type="button" class="catalog-mode-btn' +
        (state.catalogMode === "center_coaches" ? "" : " active") +
        '" data-mode="center_schedule" role="tab" aria-selected="' +
        (state.catalogMode === "center_coaches" ? "false" : "true") +
        '">Расписание</button>' +
        '<button type="button" class="catalog-mode-btn' +
        (state.catalogMode === "center_coaches" ? " active" : "") +
        '" data-mode="center_coaches" role="tab" aria-selected="' +
        (state.catalogMode === "center_coaches" ? "true" : "false") +
        '">Тренеры</button>';
      row.removeAttribute("data-bound");
  ...
  function applyCollectiveHero(brand) {
    ...
    if (!brand || !state.collectiveSlug) {
      ...
      document.documentElement.style.removeProperty(
        "--catalog-collective-accent",
      );
      document.documentElement.style.removeProperty(
        "--catalog-collective-accent-soft",
      );
      document.documentElement.style.removeProperty(
        "--catalog-collective-accent-border",
      );
      ...
      if (leadEl)
        leadEl.textContent =
          "Город и занятие — покажем свободные слоты на аренах.";
      if (aboutEl) {
        aboutEl.hidden = true;
        aboutEl.textContent = "";
      }
    ...
    if (accent.accent_soft)
      document.documentElement.style.setProperty(
        "--catalog-collective-accent-soft",
        accent.accent_soft,
      );
    if (accent.accent_border)
      document.documentElement.style.setProperty(
        "--catalog-collective-accent-border",
        accent.accent_border,
      );
    if (titleEl)
      titleEl.textContent = brand.display_name || state.collectiveSlug;
    ...
    if (leadEl) {
      if (brand.hero_variant === "center") {
        leadEl.textContent =
          brand.tagline || "Расписание центра — выберите окно и формат визита.";
      } else {
        leadEl.textContent =
          brand.tagline ||
    ...
    if (hero) {
      if (brand.cover_url) {
        ...
        hero.style.backgroundImage =
          "linear-gradient(180deg, rgba(0,0,0,0.08) 0%, rgba(0,0,0,0.55) 100%), url(" +
          brand.cover_url +
          ")";
  ...
  function centerBookingHapticSelection() {
    if (
      window.ClientShell &&
      typeof window.ClientShell.hapticSelection === "function"
    ) {
  ...
  function formatCenterSeatsLabel(seats) {
    ...
    return ruText.countRu(n, "место", "места", "мест");
  }

  function buildCenterPriceBreakdown(mode, guestCount, tariffs) {
    var guest = Number(
      (tariffs && tariffs.guest_surcharge_cents) != null
        ? tariffs.guest_surcharge_cents
        : 1500,
    );
    var guests = Math.max(0, parseInt(guestCount, 10) || 0);
    var guestLabel = ruText.countRu(guests, "гость", "гостя", "гостей");
    ...
    if (mode === "lane_with_guest") {
      return (
        ...
        guestLabel +
        " × " +
        ruText.formatPrice((guest / 100).toFixed(0), "BYN")
      );
    }
    ...
    if (mode === "center_coach_individual")
      return "Индивидуально с тренером центра";
  ...
  function formatCenterPriceByn(cents) {
    ...
    return ruText.formatPrice(
      Math.round(v) === v ? v.toFixed(0) : v.toFixed(2),
      "BYN",
    );
  }
  ...
  function computeCenterBookingPriceCents(mode, guestCount, tariffs) {
    var lane = Number(
      (tariffs && tariffs.lane_hour_cents) != null
        ? tariffs.lane_hour_cents
        : 2000,
    );
    var guest = Number(
      (tariffs && tariffs.guest_surcharge_cents) != null
    ...
    var coachInd = Number(
      (tariffs && tariffs.coach_individual_cents) != null
        ? tariffs.coach_individual_cents
        : 7000,
    );
    var coachPair = Number(
      (tariffs && tariffs.coach_pair_cents) != null
        ? tariffs.coach_pair_cents
        : 10000,
    );
    var guests = Math.max(0, parseInt(guestCount, 10) || 0);
  ...
  function centerPassKindForMode(mode) {
    if (mode === "center_coach_individual" || mode === "center_coach_pair")
      return "coach";
    if (
      mode === "lane_self" ||
      mode === "lane_with_guest" ||
      mode === "lane_own_coach"
    )
      return "lane";
    return null;
  ...
  function computeCenterBookingPriceWithPassCents(
    mode,
    guestCount,
    tariffs,
    passKind,
    ...
    if (
      passKind === "coach" &&
      (mode === "center_coach_individual" || mode === "center_coach_pair")
    ) {
      return 0;
    }
    return computeCenterBookingPriceCents(mode, guestCount, tariffs);
  ...
  function eligibleCenterPassesForMode(mode) {
    var passes =
      (state.clientCenterPasses && state.clientCenterPasses.passes) || [];
  ...
  function loadClientCenterPasses() {
    ...
    return fetch(
      "/api/webapp/client/collective-passes?collective_slug=" +
        encodeURIComponent(state.collectiveSlug),
      {
  ...
  function findCenterSessionById(sessionId) {
    var sessions =
      (state.centerSessions && state.centerSessions.sessions) || [];
  ...
  function buildCollectivePassProductCard(product) {
    ...
    return (
      '<div class="catalog-collective-pass-card" data-pass-product-id="' +
      product.id +
      '">' +
      '<div class="catalog-collective-pass-card__head">' +
      '<span class="catalog-collective-pass-card__kind">' +
      escapeHtml(kindBadge) +
      "</span>" +
      '<span class="catalog-collective-pass-card__price">' +
      escapeHtml(price) +
      "</span>" +
      "</div>" +
      '<div class="catalog-collective-pass-card__name">' +
      escapeHtml(product.name || "Абонемент") +
      "</div>" +
      '<p class="catalog-collective-pass-card__meta">' +
      escapeHtml(String(visits) + " посещений · " + validity) +
      "</p>" +
      '<button type="button" class="btn-primary catalog-collective-pass-order" data-pass-product-id="' +
      product.id +
      '">Оформить</button>' +
      "</div>"
    );
  ...
  function buildCenterPassesCatalogHtml(passProducts) {
    ...
    var lane = passProducts.filter(function (p) {
      return p.pass_kind === "lane";
    });
    var coach = passProducts.filter(function (p) {
      return p.pass_kind === "coach";
    });
    var html =
      '<div class="catalog-collective-passes">' +
    ...
    if (lane.length) {
      html +=
        '<div class="catalog-collective-passes-group">' +
  ...
  function bindCenterPassOrderButtons(panel) {
    if (!panel) return;
    panel
      .querySelectorAll(".catalog-collective-pass-order")
      .forEach(function (btn) {
        btn.addEventListener("click", function () {
          centerBookingHapticSelection();
          var productId = parseInt(
            btn.getAttribute("data-pass-product-id"),
            10,
          );
          var products =
            (state.centerSessions && state.centerSessions.pass_products) || [];
          var product = products.find(function (p) {
            return Number(p.id) === productId;
          });
          if (!product) {
            if (tg && tg.showAlert)
              tg.showAlert("Абонемент не найден — обновите страницу");
            return;
          }
  ...
  function openCenterPassOrderConfirm(product, sourceBtn) {
    ...
    if (!initData) {
      if (tg && tg.showAlert)
        tg.showAlert("Откройте каталог через клиентский бот Telegram");
      return;
    }
    state.centerPassOrderDraft = {
      product: product,
    ...
    if (textEl) {
      var visits =
        product.sessions_total != null ? product.sessions_total : "—";
      textEl.innerHTML =
        "<strong>" +
        escapeHtml(product.name || "Абонемент") +
        "</strong><br>" +
        escapeHtml(
          collectivePassKindLabel(product.pass_kind) +
            " · " +
            visits +
            " посещений · " +
            formatCollectivePassValidity(product) +
            " · " +
            formatCenterPriceByn(product.price_cents),
        );
    }
    var modal = document.getElementById("centerPassOrderConfirmModal");
  ...
  function submitCenterPassOrderRequest() {
    ...
    if (!state.collectiveSlug) {
      if (tg && tg.showAlert)
        tg.showAlert("Центр не определён — обновите страницу");
      return;
    }
    ...
    fetch("/api/webapp/client/collective-pass-order/request", {
      ...
      .then(function (o) {
        ...
        if (o.ok && o.data && o.data.success) {
          if (
            window.ClientShell &&
            typeof window.ClientShell.hapticSuccess === "function"
          ) {
            window.ClientShell.hapticSuccess();
          }
          if (tg && tg.showAlert) {
            tg.showAlert(
              "Заявка отправлена — центр свяжется с вами для оплаты",
            );
          }
          return;
        }
        ...
        if (tg && tg.showAlert)
          tg.showAlert(detail || "Не удалось отправить заявку");
      })
      .catch(function () {
        if (sendBtn) {
          sendBtn.disabled = false;
  ...
  function initCenterPassOrderModal() {
    ...
    if (cancelBtn)
      cancelBtn.addEventListener("click", closeCenterPassOrderConfirm);
    if (backdrop)
      backdrop.addEventListener("click", closeCenterPassOrderConfirm);
    if (sendBtn)
      sendBtn.addEventListener("click", submitCenterPassOrderRequest);
  }
  ...
  function renderCenterSessionsCatalog() {
    ...
    var lanePrice =
      tariffs.lane_hour_cents != null
        ? (tariffs.lane_hour_cents / 100).toFixed(0)
        : "20";
    var items = sessions.sessions || [];
    var passProducts = (sessions.pass_products || []).filter(function (p) {
      return p.is_active !== false;
    });
    var html = "";
    if (items.length) {
      html +=
        '<div class="catalog-collective-sessions-block">' +
        '<div class="catalog-collective-sessions-title">Запись в центр</div>' +
        '<p class="catalog-collective-sessions-hint">Выберите окно и формат — дорожка от ' +
        lanePrice +
        " BYN, гость +15 BYN</p>" +
        items
          .map(function (s) {
            var coaches = (s.assigned_coaches || [])
              .map(function (c) {
            ...
            var seats =
              s.seats_left != null
                ? s.seats_left
                : Math.max(0, (s.capacity || 1) - (s.booked_count || 0));
            return (
              '<div class="catalog-collective-session-card" data-session-id="' +
              s.id +
              '">' +
              '<div class="catalog-collective-session-card__head">' +
              '<div class="catalog-collective-session-card__when">' +
              escapeHtml(formatCenterSessionWhen(s)) +
              "</div>" +
              '<span class="catalog-collective-session-card__seats">' +
              escapeHtml(formatCenterSeatsLabel(seats)) +
              "</span>" +
              "</div>" +
              '<p class="catalog-collective-sessions-coaches">' +
              coachLine +
              "</p>" +
              '<button type="button" class="btn-primary catalog-collective-session-book" data-session-id="' +
              s.id +
              '">Выбрать формат</button>' +
              "</div>"
            );
          })
          .join("") +
      ...
      html +=
        '<div class="catalog-collective-sessions-empty">Свободных окон и абонементов пока нет — загляните позже.</div>';
    } else {
      html +=
        '<div class="catalog-collective-sessions-empty catalog-collective-sessions-empty--compact">Свободных окон пока нет — ниже можно оформить абонемент.</div>';
    }
    html += buildCenterPassesCatalogHtml(passProducts);
    panel.innerHTML = html;
    panel
      .querySelectorAll(".catalog-collective-session-book")
      .forEach(function (btn) {
        btn.addEventListener("click", function () {
          centerBookingHapticSelection();
          openCenterSessionBookingModal(
            parseInt(btn.getAttribute("data-session-id"), 10),
          );
        });
      });
  ...
  function openCenterSessionBookingModal(sessionId) {
    ...
    if (!session) {
      if (tg && tg.showAlert)
        tg.showAlert("Окно не найдено — обновите страницу");
      return;
    }
    ...
    if (
      window.ClientShell &&
      typeof window.ClientShell.setTabBarVisible === "function"
    ) {
  ...
  function getCenterBookingModeDef(modeId) {
    for (var i = 0; i < CENTER_ATTENDANCE_MODES.length; i++) {
      if (CENTER_ATTENDANCE_MODES[i].id === modeId)
        return CENTER_ATTENDANCE_MODES[i];
    }
    return null;
  }
  ...
  function renderCenterBookingModal() {
    ...
    if (modeListEl) {
      modeListEl.innerHTML = modes
        .map(function (m) {
          var guestPreview = 0;
          if (m.id === "center_coach_pair") guestPreview = 1;
          else if (m.needsGuests)
            guestPreview = Math.max(m.minGuests || 1, draft.guestCount || 1);
          else if (m.optionalGuests)
            guestPreview = Math.max(0, draft.guestCount || 0);
          var price = computeCenterBookingPriceCents(
            m.id,
            guestPreview,
            tariffs,
          );
          ...
          return (
            '<button type="button" class="center-booking-mode' +
            active +
            '" data-mode="' +
            m.id +
            ...
            (active ? "true" : "false") +
            '">' +
            '<span class="center-booking-mode__emoji" aria-hidden="true">' +
            (m.emoji || "•") +
            "</span>" +
            '<span class="center-booking-mode__main">' +
            '<span class="center-booking-mode__title">' +
            escapeHtml(m.title) +
            "</span>" +
            '<span class="center-booking-mode__subtitle">' +
            escapeHtml(m.subtitle) +
            "</span>" +
            "</span>" +
            '<span class="center-booking-mode__price">' +
            escapeHtml(formatCenterPriceByn(price)) +
            "</span>" +
            "</button>"
          );
        })
        ...
        .forEach(function (btn) {
          btn.addEventListener("click", function () {
            ...
            if (def && def.coachMode) {
              var coaches = session.assigned_coaches || [];
              if (coaches.length === 1)
                draft.centerCoachId = coaches[0].trainer_id;
            }
            var eligible = eligibleCenterPassesForMode(draft.mode);
            if (
              draft.passInstanceId &&
              !eligible.some(function (p) {
                return Number(p.id) === Number(draft.passInstanceId);
              })
            ) {
              draft.passInstanceId =
    ...
    if (coachSection && coachListEl) {
      ...
      if (showCoach) {
        coachListEl.innerHTML = coaches
          .map(function (c) {
            var active =
              Number(draft.centerCoachId) === Number(c.trainer_id)
                ? " is-active"
                : "";
            return (
              '<button type="button" class="center-booking-coach' +
              active +
              '" data-coach-id="' +
              c.trainer_id +
              '">' +
          ...
          .join("");
        coachListEl
          .querySelectorAll(".center-booking-coach")
          .forEach(function (btn) {
            btn.addEventListener("click", function () {
              centerBookingHapticSelection();
              draft.centerCoachId = parseInt(
                btn.getAttribute("data-coach-id"),
                10,
              );
              renderCenterBookingModal();
            });
    ...
    var showGuests = !!(
      modeDef &&
      (modeDef.needsGuests || modeDef.optionalGuests)
    );
    if (guestSection) guestSection.hidden = !showGuests;
    if (showGuests) {
      if (guestLabelEl) {
        guestLabelEl.textContent = modeDef.optionalGuests
    ...
    if (passSection && passListEl) {
      ...
      if (eligiblePasses.length) {
        var passButtons = [
          '<button type="button" class="center-booking-coach' +
            (!draft.passInstanceId ? " is-active" : "") +
            '" data-pass-id="">Без абонемента · PAYG</button>',
        ].concat(
          eligiblePasses.map(function (p) {
            var active =
              Number(draft.passInstanceId) === Number(p.id) ? " is-active" : "";
            var label =
              (p.product_name || "Абонемент") +
              " · " +
    ...
    if (modeDef) {
      ...
      else if (modeDef.needsGuests)
        guestCountForPrice = Math.max(modeDef.minGuests || 1, draft.guestCount);
      else if (modeDef.optionalGuests)
        guestCountForPrice = Math.max(0, draft.guestCount);
    }
    ...
    if (priceEl) {
      priceEl.textContent =
        draft.passInstanceId && totalCents === 0
          ? "По абонементу"
    ...
    if (breakdownEl) {
      if (draft.passInstanceId) {
        ...
        var extra =
          totalCents > 0
            ? " · доплата " + formatCenterPriceByn(totalCents)
            : "";
        breakdownEl.textContent =
          "Списание " + credits + " посещ. с абонемента" + extra;
      } else {
        breakdownEl.textContent = buildCenterPriceBreakdown(
          draft.mode,
          guestCountForPrice,
          tariffs,
        );
    ...
    if (submitBtn) {
      ...
      if (
        modeDef &&
        modeDef.needsGuests &&
        draft.guestCount < (modeDef.minGuests || 1)
      )
        canSubmit = false;
  ...
  function submitCenterSessionBooking() {
    ...
    if (modeDef) {
      if (modeDef.id === "center_coach_pair") guestCount = 1;
        ...
        guestCount = Math.max(modeDef.minGuests || 1, draft.guestCount);
      else if (modeDef.optionalGuests)
        guestCount = Math.max(0, draft.guestCount);
    }
    var commentEl = document.getElementById("centerBookingComment");
    ...
    fetch("/api/webapp/client/collective-session-booking", {
      ...
      .then(function () {
        ...
        if (tg && tg.showAlert)
          tg.showAlert("Заявка отправлена — центр подтвердит запись");
      ...
      .catch(function (e) {
        ...
        if (tg && tg.showAlert)
          tg.showAlert(e.message || "Не удалось записаться");
      });
  }
  ...
  function initCenterBookingModal() {
    ...
    var commentEl = document.getElementById("centerBookingComment");
    if (closeBtn)
      closeBtn.addEventListener("click", closeCenterSessionBookingModal);
    if (backdrop)
      backdrop.addEventListener("click", closeCenterSessionBookingModal);
    if (modal) {
    ...
    if (submitBtn)
      submitBtn.addEventListener("click", submitCenterSessionBooking);
    if (commentEl) {
      commentEl.addEventListener("input", function () {
        if (state.centerBookingDraft)
          state.centerBookingDraft.comment = commentEl.value;
      });
    }
  }
  ...
  function loadCenterSessionsCatalog() {
    if (
      !state.collectiveSlug ||
      !state.collectiveBrand ||
      !isCenterCatalogBrand(state.collectiveBrand)
    ) {
    ...
    return fetch(
      ...
      .then(function (res) {
        return res.json().then(function (data) {
          if (!res.ok)
            throw new Error((data && data.detail) || "sessions_failed");
          return data;
        });
      })
      .then(function (data) {
  ...
  function bootstrapCollectiveFromQuery(qp) {
    ...
    return fetch("/api/public/collectives/" + encodeURIComponent(slug), {
      cache: "no-store",
    })
      .then(function (res) {
        return res.json().then(function (data) {
          if (!res.ok)
            throw new Error((data && data.detail) || "collective_not_found");
          return data;
        });
      })
      .then(function (data) {
        ...
        return loadCenterSessionsCatalog().then(function () {
          return data;
        });
      })
      .catch(function () {
  ...
  function updateCatalogModeUi() {
    ...
    if (titleEl) {
      ...
      else if (mode === "center_coaches")
        titleEl.textContent = "Тренеры центра";
    ...
    document
      .querySelectorAll("#catalogListMode .catalog-mode-btn")
      .forEach(function (btn) {
    ...
    row.querySelectorAll(".catalog-mode-btn").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (mode === "groups") {
          ...
          document
            .querySelectorAll("#timeSlotFilters .filter-chip")
  ...
  function initializeFilterHandlers() {
    ...
    // Day filters
    document
      .querySelectorAll("#dayFilters .filter-chip")
      .forEach(function (chip) {
  ...
  function lookupServiceTrainerCount(serviceId, cityId) {
    ...
    var items =
      _catalogServicesCacheByCity[catalogServicesCacheKey(cityId)] || [];
  ...
  function refreshCatalogOfferTrainerCount() {
    ...
    return getJson("/trainers", makeTrainerListParams(0))
      .then(function (data) {
        ...
        var total =
          data.total != null ? data.total | 0 : (data.items || []).length;
  ...
  function patchFilterTileOfferMeta(action, label) {
    var tile = document.querySelector(
      '#summaryRows .catalog-filter-tile[data-action="' + action + '"]',
    );
    if (!tile) return;
  ...
  function prefetchFirstPageIfNeeded() {
    ...
    var path =
      state.catalogMode === "groups" ? "/training-groups" : "/trainers";
    var params =
      state.catalogMode === "groups"
        ? makeGroupListParams(0)
        : makeTrainerListParams(0);
    fetchCatalogFirstPage(path, params, key)
      .then(function (data) {
        ...
        if (state.arenaIds.length > 0 && state.cityId && state.serviceId) {
          ...
          state.catalogOfferCount =
            data.total != null ? data.total | 0 : items.length;
  ...
  function normalizeCatalogEducationDocumentPhotos(edu) {
    var rows =
      edu && Array.isArray(edu.document_photos) ? edu.document_photos : [];
    var out = [];
    var seen = {};
  ...
  function buildCatalogEducationItemHtml(edu) {
    ...
    if (typeHint) {
      body +=
        '<div class="trainer-detail-education-body-line"><b>Тип:</b> ' +
        escapeHtml(typeHint) +
        "</div>";
    }
    if (degree) {
      body +=
        '<div class="trainer-detail-education-body-line"><b>Квалификация:</b> ' +
        escapeHtml(degree) +
        "</div>";
    }
    if (years) {
      body +=
        '<div class="trainer-detail-education-body-line"><b>Период:</b> ' +
        escapeHtml(years) +
        "</div>";
    }
    if (place) {
      body +=
        '<div class="trainer-detail-education-body-line"><b>Город:</b> ' +
        escapeHtml(place) +
        "</div>";
    }
    if (docs.length) {
      var docsHtml = docs
        ...
        .map(function (doc, idx) {
          ...
          return (
            '<a class="trainer-detail-education-doc" href="' +
            href +
            '" target="_blank" rel="noopener noreferrer" aria-label="Документ #' +
            (idx + 1) +
            '">' +
            '<img src="' +
            imgSrc +
      ...
      if (docs.length > 4) {
        docsHtml +=
          '<span class="trainer-detail-education-doc-more">+' +
          (docs.length - 4) +
          "</span>";
      }
      body +=
        '<div class="trainer-detail-education-docs">' + docsHtml + "</div>";
    }
    if (!body) {
      body =
        '<div class="trainer-detail-education-body-line">Без дополнительных деталей.</div>';
    }
    ...
    return (
      ...
      '<div class="trainer-detail-education-summary-title">' +
      escapeHtml(summary) +
      "</div>" +
      (summaryMeta
        ? '<div class="trainer-detail-education-summary-meta">' +
      ...
      (docs.length
        ? '<span class="trainer-detail-education-badge">' +
          docs.length +
          " фото</span>"
        : "") +
      ...
      "</div>" +
      "</summary>" +
      '<div class="trainer-detail-education-body">' +
      body +
      "</div>" +
      "</details>"
    );
  ...
  function buildCatalogEducationSectionInner(
    eduRows,
    profileEduDetail,
    ...
    if (eduRows.length > 0) {
      eduSectionInner +=
        '<p class="trainer-detail-education-hint">Нажмите на запись, чтобы раскрыть детали и фото документов.</p>';
      var eduAnyHtml = "";
      ...
      if (eduAnyHtml) {
        ...
        eduSectionInner +=
          '<div class="trainer-detail-education-body-line">' +
          escapeHtml(profileEduDetail).replace(/\n/g, "<br>") +
          "</div>";
      }
      if (profileExtraForDetail) {
        eduSectionInner += '<div class="trainer-detail-education-extra">';
        eduSectionInner +=
          '<div class="trainer-detail-education-extra-label">Дополнительно</div>';
        eduSectionInner +=
          '<div class="trainer-detail-education-extra-body">' +
          escapeHtml(profileExtraForDetail).replace(/\n/g, "<br>") +
          "</div>";
        eduSectionInner += "</div>";
      }
    } else if (profileEduDetail) {
  ...
  function buildTrainerAboutSectionHtml(
    desc,
    eduRows,
    profileEduDetail,
    profileExtraForDetail,
    ...
    var hasDesc = desc !== "—";
    var eduSectionInner = buildCatalogEducationSectionInner(
      eduRows,
      profileEduDetail,
      profileExtraForDetail,
    );
    ...
    var hasEduPreview =
      eduPreview.length > 0 || !!profileEduChip || !!profileEduLine;
    ...
    if (eduSectionInner) {
      needsEduDetails =
        ...
        !!profileExtraForDetail ||
        (profileEduDetail &&
          !isProfileEducationCategoryOnly(profileEduDetail) &&
          eduRows.length === 0 &&
          (profileEduDetail.length > 120 ||
            profileEduDetail.indexOf("\n") >= 0));
    }
    ...
    if (hasDesc) {
      ...
      if (catalogBioNeedsExpand(desc)) {
        ...
        html +=
          '<p class="trainer-detail-about-bio trainer-detail-about-bio--clamp">' +
          bioHtml +
          "</p>";
        html += '<details class="trainer-detail-about-bio-more">';
        html +=
          '<summary class="trainer-detail-about-bio-more-summary">Читать полностью</summary>';
        html +=
          '<div class="trainer-detail-about-bio trainer-detail-about-bio--full">' +
          bioHtml +
          "</div>";
        html += "</details></div>";
      } else {
    ...
    if (hasEduPreview) {
      ...
      if (profileEduChip) {
        html +=
          '<span class="trainer-detail-about-edu-chip">' +
          escapeHtml(profileEduChip) +
          "</span>";
      } else {
        eduPreview.forEach(function (line) {
          html +=
            '<div class="trainer-detail-about-edu-line">' +
            escapeHtml(line) +
            "</div>";
        });
        if (profileEduLine && eduPreview.length === 0) {
          html +=
            '<div class="trainer-detail-about-edu-line">' +
            escapeHtml(profileEduLine) +
            "</div>";
        }
        if (eduHiddenCount > 0) {
          html +=
            '<div class="trainer-detail-about-edu-more">Ещё ' +
            eduHiddenCount +
            " " +
            (eduHiddenCount === 1 ? "запись" : "записи") +
            "</div>";
        }
      }
      html += "</div>";
  ...
  function bindTrainerDetailEducationDocLinks(rootEl) {
    if (!rootEl) return;
    rootEl
      .querySelectorAll(".trainer-detail-education-doc")
  ...
  function openTrainerDetailFromTrainerObject(t, prefetchPhotos) {
    ...
    if (prefetchPhotos) {
      ...
      if (pf && pf !== pl) {
        var imf = new Image();
        imf.decoding = "async";
        imf.fetchPriority = "high";
        imf.src = pf;
      }
    }
    paintTrainerDetailSkeleton();
    showScreen("screenTrainerDetail");
    loadTrainerById(t.id)
      .then(function (full) {
        state.selectedTrainer = full || t;
        state.trainerName = trainerName(state.selectedTrainer);
        reconcileCatalogServiceWithTrainerAsync(state.selectedTrainer).then(
          function () {
  ...
  function getJson(path, params) {
    ...
    return fetch("/api/public" + path + q, { cache: "no-store" }).then(
      function (r) {
  ...
  function loadTrainerByIdViaWebappFallback(trainerId) {
    ...
    if (!initData) return Promise.resolve(null);
    return fetch(
      "/api/webapp/client/catalog-trainer/" + encodeURIComponent(trainerId),
      {
        headers: { "X-Telegram-Init-Data": initData },
        cache: "no-store",
      },
  ...
  function loadTrainerById(trainerId) {
    ...
    if (initData) {
      return loadTrainerByIdViaWebappFallback(trainerId).then(function (t) {
        if (t) return t;
        return fetch("/api/public/trainers/" + encodeURIComponent(trainerId), {
          cache: "no-store",
        })
          .then(function (r) {
            if (r.status === 404) return null;
  ...
  function switchTab(tab) {
    ...
    document
      .getElementById("catalogTabPanel")
      .classList.toggle("active", tab === "catalog");
    document
      .getElementById("myTrainerTabPanel")
      .classList.toggle("active", tab === "my_trainer");
    if (tab === "catalog") {
  ...
  function openMyTrainerCard() {
    ...
    paintTrainerDetailSkeleton();
    showScreen("screenTrainerDetail");
    loadTrainerById(state.trainerId)
      .then(function (t) {
        document.getElementById("myTrainerLoading").style.display = "none";
        if (!t) {
  ...
  function getClientSession(opts) {
    ...
    return fetch("/api/webapp/client/session" + q, {
      headers: headers,
      cache: "no-store",
    }).then(function (r) {
      if (!r.ok) throw new Error(r.statusText);
  ...
  function reconcileCatalogServiceWithTrainerAsync(trainer) {
    return new Promise(function (resolve) {
      ...
      function applyPick(sid, doPersist) {
        ...
        state.serviceId = num;
        state.serviceName = String(allowed[num].service_name || "").trim();
        if (doPersist && state.cityId && state.serviceId)
          persistTrainerSelection(trainer.id);
        return true;
      }
      ...
      if (!isNaN(cur) && allowed[cur]) {
        if (
          isHubDirectBookEntry() &&
          applyCatalogRepeatBookingDefaults(trainer)
        ) {
          if (state.cityId && state.serviceId)
            persistCatalogFilters(trainer.id);
          resolve();
          return;
        }
        state.serviceName = String(
          allowed[cur].service_name || state.serviceName || "",
        ).trim();
        ...
        .then(function (sess) {
          if (sess) {
            ...
            state.suggestedServiceForTrainer =
              sidRaw != null && sidRaw !== "" && !isNaN(Number(sidRaw))
                ? Number(sidRaw)
                : null;
            if (
              state.suggestedServiceForTrainer != null &&
              state.suggestedServiceForTrainerId != null &&
              Number(state.suggestedServiceForTrainerId) ===
                Number(trainer.id) &&
          ...
          var zid =
            pick0 && pick0.service_id != null ? Number(pick0.service_id) : NaN;
          if (!isNaN(zid)) {
            state.serviceId = zid;
    ...
    if (data.primary && data.primary.trainer_id != null) {
      ...
      if (!isNaN(pt) && pt > 0) {
        ...
        if (dn != null && String(dn).trim())
          state.trainerName = String(dn).trim();
      }
    }
  }
  ...
  function toggleSaveTrainer(trainerId) {
    ...
    fetch(url, { method: method, headers: headers, body: body })
      ...
      .catch(function () {
        ...
        state.trainerEdges[key] = Object.assign({}, edge, {
          is_saved: nowSaved,
        });
  ...
  function _updateSaveButtonUI(trainerId, isSaved) {
    ...
    chip.querySelector("span").textContent = isSaved
      ? "Сохранено"
      : "Сохранить";
  ...
  function _updateNotifyChipUI(trainerId, isNotify) {
    ...
    if (!chip || parseInt(chip.dataset.trainerId, 10) !== trainerId) return;
    chip.querySelector("span").textContent = isNotify
      ? "Подписан"
      : "Напомнить";
    ...
    if (hint) {
      ...
      if (isNotify) {
        ...
        hint.textContent =
          "Нет слотов — нажмите 🔕, чтобы получить уведомление";
      } else {
  ...
  function toggleNotifySlots(trainerId) {
    ...
    var body = nextNotify
      ? JSON.stringify({ trainer_id: trainerId })
      : undefined;
    fetch(url, { method: method, headers: headers, body: body })
      .then(function (r) {
        return r.ok ? r.json() : Promise.reject(r.status);
  ...
  function trainerContactTelegramUrl(trainer) {
    var raw =
      trainer && typeof trainer.contact_telegram_url === "string"
  ...
  function openTrainerTelegramContact(trainer) {
    ...
    if (typeof window.openTelegramChatFromMiniApp === "function") {
      ...
      return window.openTelegramChatFromMiniApp({
        username: un,
        telegramId:
          trainer && trainer.telegram_id != null ? trainer.telegram_id : null,
      });
    }
    return false;
  }
  ...
  function appendContactTrainerButton(container, trainer, opts) {
    opts = opts || {};
    if (!container || !trainer || !trainerContactTelegramUrl(trainer))
      return false;
    var btn = document.createElement("button");
    btn.type = "button";
    btn.className =
      (opts.primary ? "btn-primary" : "btn-secondary") +
      " btn-block trainer-contact-btn";
  ...
  function appendTrainerActionChips(container, trainer, noSlots) {
    ...
    chipNotify.className =
      "trainer-action-chip trainer-action-chip--notify" +
      (isNotify ? " is-active" : "") +
    ...
    chipNotify.innerHTML =
      ...
      "</span>" +
      "<span>" +
      (isNotify ? "Подписан" : "Напомнить") +
      "</span>";
    chipNotify.onclick = function () {
      toggleNotifySlots(tid);
    };
    ...
    chipSave.className =
      "trainer-action-chip trainer-action-chip--save" +
      (isSaved ? " is-active" : "");
    chipSave.innerHTML =
      '<span class="chip-icon">' +
      (isSaved ? "❤️" : "🤍") +
      ...
      "</span>";
    chipSave.onclick = function () {
      toggleSaveTrainer(tid);
    };
  ...
  function applySessionToState(session, opts) {
    ...
    if (!bookingOnly) {
      ...
      if (session.trainer_id) {
        state.trainerId = session.trainer_id;
        state.trainerName = session.trainer_name || "";
      } else {
        state.trainerId = null;
        state.trainerName = "";
      }
    ...
    state.clientFirstName =
      session.client_first_name != null &&
      session.client_first_name !== undefined
  ...
  function refreshClientPhoneForBookingForm(cb) {
    ...
    var req =
      tid != null
        ? getClientSession({ forTrainerId: tid })
        : getClientSession();
    req
      .then(function (session) {
        applySessionToState(
          session,
          tid != null ? { bookingFormRefresh: true } : undefined,
        );
        updateBookingNameFieldsVisibility();
        if (cb) cb();
      })
  ...
  function clearServiceIfInvalidForCity(cityId) {
    if (!state.serviceId && !state.catalogScenarioStub)
      return Promise.resolve();
  ...
  function wirePhoneCallLinks(root) {
    var Pf = window.CrmPhoneField;
    if (!Pf) return;
    if (typeof Pf.wirePhoneCallButtons === "function")
      Pf.wirePhoneCallButtons(root || document);
    else if (typeof Pf.wirePhoneTelLinks === "function")
      Pf.wirePhoneTelLinks(root || document);
  }
  ...
  function syncPickerSearchChrome(kind, totalCount, visibleCount, query) {
    ...
    if (meta) {
      if (!showSearch || !hasQuery) {
        ...
        meta.textContent =
          "Ничего не найдено — попробуйте другое название или адрес";
  ...
  function bindPickerSearchOnce(kind, onInput) {
    var boundFlag =
      kind === "city" ? catalogCitySearchBound : catalogArenaSearchBound;
    if (boundFlag) return;
  ...
  function formatCatalogGroupRules(rules) {
    ...
    return rules
      .map(function (r) {
        return (
          formatCatalogGroupDow(r.day_of_week) +
          " " +
  ...
  function loadTrainingGroupsBlock(trainer) {
    ...
    el.innerHTML = "";
    getJson("/trainers/" + trainer.id + "/training-groups")
      .then(function (data) {
        ...
        groups.forEach(function (g) {
          ...
          html +=
            '<div class="trainer-detail-group-name">' +
            escapeHtml(g.name || "") +
            "</div>";
          html +=
            '<div class="trainer-detail-group-meta">' +
            escapeHtml(g.service_name || "—");
          if (g.arena_name) html += " · " + escapeHtml(g.arena_name);
          html += "</div>";
          if (sched)
            html +=
              '<div class="trainer-detail-group-sched">' +
              escapeHtml(sched) +
              "</div>";
          if (g.catalog_pitch)
            html +=
              '<div class="trainer-detail-group-pitch">' +
              escapeHtml(g.catalog_pitch) +
              "</div>";
          html +=
            '<div class="trainer-detail-group-spots">Занято: ' +
            (g.seated_count != null ? g.seated_count : 0) +
            " из " +
            (g.max_members || "—") +
            " · свободно " +
            (g.spots_left != null ? g.spots_left : "—") +
            "</div>";
          html +=
            '<button type="button" class="btn-primary btn-block catalog-group-join" data-group-id="' +
            g.id +
  ...
  function submitCatalogGroupJoinRequest(groupId) {
    ...
    var url =
      "/api/webapp/client/training-groups/" +
      groupId +
      "/join-request?initData=" +
      encodeURIComponent(initData);
      ...
      .then(function (r) {
        return r.json().then(function (j) {
          if (!r.ok) {
            var d = j && j.detail;
            throw new Error(
              typeof d === "string" ? d : r.statusText || "Ошибка",
            );
          }
          return j;
        });
  ...
  var CATALOG_FILTER_SHORT_LABELS = {
    city: "Город",
    service: "Что ищете",
    arena: "Площадка",
  };
  ...
  function fetchCatalogArenas(cityId, serviceId) {
    ...
    if (_catalogArenasCache[key])
      return Promise.resolve(_catalogArenasCache[key]);
  ...
  function fetchCatalogServices(cityId) {
    ...
    if (_catalogServicesCacheByCity[key])
      return Promise.resolve(_catalogServicesCacheByCity[key]);
  ...
  function syncScenarioChipSelection() {
    ...
    if (!stubKey && state.serviceId) {
      var items =
        _catalogServicesCacheByCity[
          catalogServicesCacheKey(state.cityId || null)
        ];
      if (items) {
        for (var k in CATALOG_SCENARIO_STUBS) {
          if (!Object.prototype.hasOwnProperty.call(CATALOG_SCENARIO_STUBS, k))
            continue;
  ...
  function renderCatalogFilterSkeleton() {
    ...
    var html =
      '<div class="catalog-filter-skel-list" role="status" aria-busy="true" aria-label="Загрузка параметров">';
    for (var i = 0; i < 3; i++) {
      html +=
  ...
  function updateCatalogHomeMode() {
    ...
    if (state.cityId && state.serviceId) {
      ...
      leadEl.textContent =
        "Нажмите «Найти тренера» — покажем подходящих специалистов.";
    } else {
  ...
  function wireCatalogScenarioChips() {
    ...
    host.querySelectorAll(".catalog-scenario-chip").forEach(function (chip) {
      ...
      chip.addEventListener("click", function () {
        ...
        if (!stub) return;
        var wasSelected = chip.classList.contains(
          "catalog-scenario-chip--selected",
        );
        host.querySelectorAll(".catalog-scenario-chip").forEach(function (c) {
  ...
  function catalogServiceSummaryDisplay() {
    ...
    var stub =
      state.catalogScenarioStub &&
      CATALOG_SCENARIO_STUBS[state.catalogScenarioStub];
  ...
  function updateCatalogFindUi() {
    ...
    if (labelEl)
      labelEl.textContent = hasCity ? CATALOG_CTA_LABEL : "Выбрать город";
  ...
  function renderSummary() {
    ...
    var rows = [
      { key: "city", value: state.cityName || "", scenario: false },
      {
        ...
        scenario: !state.serviceName && !!state.catalogScenarioStub,
      },
    ...
    document.getElementById("summaryRows").innerHTML = rows
      .map(function (r) {
        ...
        var tileCls =
          "catalog-filter-tile" +
          (isSet
            ? " catalog-filter-tile--set"
            : " catalog-filter-tile--empty") +
        ...
        return (
          ...
          "</span>" +
          (offerMeta
            ? '<span class="catalog-filter-tile__meta">' +
              escapeHtml(offerMeta) +
              "</span>"
            : "") +
          "</span>" +
          (isSet
    ...
    document
      .querySelectorAll("#summaryRows .catalog-filter-tile")
      .forEach(function (btn) {
        btn.onclick = function () {
      ...
      .then(function (data) {
        ...
        if (!catalogCityItems.length) {
          document.getElementById("cityList").innerHTML =
            '<div class="empty">Нет городов</div>';
  ...
  function renderCityList() {
    ...
    var html = filtered
      .map(function (c) {
        return (
          '<button type="button" class="choice-card" data-id="' +
          c.id +
          '" data-name="' +
          escapeAttr(c.name || "") +
          '">' +
          '<div class="main"><div class="label">' +
          CATALOG_FILTER_LABELS.city +
          '</div><div class="value">' +
          escapeHtml(c.name || "") +
          "</div></div>" +
  ...
  function openServiceSummaryModal(title, bodyText) {
    ...
    modalBody.textContent =
      bodyText != null && bodyText !== "" ? String(bodyText) : "";
  ...
  function loadServices() {
    ...
    if (titleEl) {
      titleEl.textContent = state.cityName
        ? "Занятия · " + state.cityName
        : "Что ищете";
    }
    ...
    loadReq
      .then(function (data) {
        ...
        if (!items.length) {
          ...
          if (backCityEmpty)
            backCityEmpty.style.display = state.returnToSummary
              ? "none"
              : "block";
          syncCatalogHeaderBack();
          return;
        ...
        items.forEach(function (s) {
          if (
            s.client_summary != null &&
            String(s.client_summary).trim() !== ""
          ) {
          ...
          .map(function (s) {
            ...
            var offerLine = showCount
              ? '<div class="choice-card__meta">' +
                escapeHtml(formatServiceOfferCount(count)) +
                "</div>"
              : "";
        ...
        if (backCity)
          backCity.style.display = state.returnToSummary ? "none" : "block";
        document
          .querySelectorAll("#serviceList .choice-card")
          .forEach(function (btn) {
            btn.onclick = function () {
              ...
              if (chipHost) {
                chipHost
                  .querySelectorAll(".catalog-scenario-chip")
                  .forEach(function (c) {
                    c.classList.remove("catalog-scenario-chip--selected");
                    c.setAttribute("aria-pressed", "false");
                  });
        ...
        document
          .querySelectorAll("#serviceList .service-info-btn")
          .forEach(function (btn) {
      ...
      .catch(function () {
        ...
        if (backCityErr)
          backCityErr.style.display = state.returnToSummary ? "none" : "block";
        syncCatalogHeaderBack();
      });
  }
  ...
  function ensureArenaMapMounted() {
    ...
    if (!wrap || !node) return;
    var items = (arenaScreenDraft.items || []).filter(function (a) {
  ...
  function mountArenaMap(items, wrap, node) {
    ...
    if (arenaMapInstance) {
      try {
        arenaMapInstance.remove();
      } catch (e) {
        /* ignore — node will be re-used */
      }
    ...
    if (
      ...
      typeof map.attributionControl.setPrefix === "function"
    ) {
    ...
    var bounds = L.latLngBounds(
      items.map(function (a) {
        return [parseFloat(a.latitude), parseFloat(a.longitude)];
      }),
    );
  ...
  function loadArenas() {
    ...
    fetchCatalogArenas(state.cityId, state.serviceId)
      .then(function (items) {
        ...
        if (backSvc)
          backSvc.style.display = state.returnToSummary ? "none" : "block";
        syncCatalogHeaderBack();
      })
      .catch(function () {
  ...
  function sortArenasSelectedFirst(filtered, selectedSet) {
    ...
    selected.sort(function (a, b) {
      return (
        arenaScreenDraft.ids.indexOf(a.id) - arenaScreenDraft.ids.indexOf(b.id)
      );
    });
  ...
  function pulseArenaCardAfterPromote(card) {
    ...
    setTimeout(function () {
      card.classList.remove("arena-card--just-selected");
    }, 700);
  }
  ...
  function renderArenaListCheckboxes(opts) {
    ...
    if (!items.length) {
      listEl.innerHTML =
        '<div class="empty">В этом городе арены пока не добавлены</div>';
    ...
    var selectedSet = {};
    arenaScreenDraft.ids.forEach(function (id) {
      selectedSet[id] = true;
    });
    ...
    var html = filtered
      .map(function (a) {
        ...
        var addr = a.address
          ? '<div class="arena-card__addr">' + escapeHtml(a.address) + "</div>"
          : "";
        ...
        var noOffers = showCount && count === 0;
        return (
          '<label class="arena-card' +
          (isSel ? " selected" : "") +
          (noOffers ? " arena-card--no-offers" : "") +
          '" data-id="' +
          a.id +
          '" data-name="' +
          name +
  ...
  function updateArenaScreenChrome() {
    ...
    if (counter)
      counter.textContent = n === 0 ? "Любая арена" : "Выбрано: " + n;
  ...
  function commitArenaScreenSelection() {
    setArenaSelection(
      arenaScreenDraft.ids.slice(),
      arenaScreenDraft.names.slice(),
    );
    state.offset = 0;
    clearCatalogSessionStorageCache();
    persistCatalogFilters();
  ...
  function formatCatalogServicePrice(s) {
    ...
    if (minV != null && maxV != null && minV !== maxV) {
      ...
      return "от " + escapeHtml(ruText.formatPrice(a, "BYN"));
    }
    var v = minV != null ? minV : s.price_byn;
    var numStr = v === Math.floor(v) ? String(v) : v.toFixed(2);
    return escapeHtml(ruText.formatPrice(numStr, "BYN"));
  }
      ...
      .then(function (data) {
        ...
        var html = items
          .map(function (g, idx) {
            ...
            var proxyFallback = photo
              ? photoUrl(photo.file_key_list || photo.file_key)
              : "";
            ...
            var meta = [
              ...
              .join(" · ");
            var spots =
              g.spots_left != null && g.max_members != null
                ? "Свободно " + g.spots_left + " из " + g.max_members
                : "";
            var photoHtml = src
              ? '<div class="cg-photo-wrap"><img class="cg-photo" src="' +
                src +
                '" alt="" loading="lazy" decoding="async" data-fallback="' +
                (proxyFallback || "").replace(/"/g, "&quot;") +
                '"></div>'
            ...
            var joinBtn =
              ...
              '">Вступить в группу</button>';
            var profileBtn =
              '<button type="button" class="btn-secondary btn-block catalog-group-open-trainer" data-index="' +
              idx +
              '">Профиль тренера</button>';
            return (
              '<div class="catalog-group-list-card" data-index="' +
              idx +
              '">' +
              photoHtml +
              '<div class="cg-body">' +
              '<div class="cg-name">' +
              escapeHtml(g.name || "Группа") +
              "</div>" +
              '<div class="cg-meta">' +
              escapeHtml(meta) +
              "</div>" +
              (spots
                ? '<div class="cg-meta">' + escapeHtml(spots) + "</div>"
                : "") +
              (g.catalog_pitch
                ? '<div class="cg-meta">' +
                  escapeHtml(
                    g.catalog_pitch.length > 120
                      ? g.catalog_pitch.slice(0, 117) + "…"
                      : g.catalog_pitch,
                  ) +
                  "</div>"
                : "") +
        ...
        document
          .querySelectorAll("#trainerList .cg-photo-wrap img.cg-photo")
        ...
        document
          .querySelectorAll("#trainerList .catalog-group-join")
          .forEach(function (btn) {
            btn.addEventListener("click", function (ev) {
              ev.stopPropagation();
              var gid = parseInt(btn.getAttribute("data-group-id"), 10);
        ...
        document
          .querySelectorAll("#trainerList .catalog-group-open-trainer")
          .forEach(function (btn) {
            btn.addEventListener("click", function (ev) {
              ev.stopPropagation();
              ...
              if (row && row.trainer)
                openTrainerDetailFromTrainerObject(row.trainer, true);
            });
          });
        document
          .querySelectorAll("#trainerList .catalog-group-list-card")
          .forEach(function (card) {
            card.addEventListener("click", function (ev) {
              if (
                ev.target.closest &&
                (ev.target.closest(".catalog-group-join") ||
                  ev.target.closest(".catalog-group-open-trainer"))
              )
                return;
        ...
        if (state.offset > 0) {
          pagHtml +=
            '<button type="button" id="pagePrevGroups">◀ Назад</button>';
        }
        pagHtml += "<span>Стр. " + page + " из " + totalPages + "</span>";
        if (state.offset + items.length < total) {
          pagHtml +=
            '<button type="button" id="pageNextGroups">Вперёд ▶</button>';
        }
        document.getElementById("pagination").innerHTML = pagHtml;
        var prevG = document.getElementById("pagePrevGroups");
        if (prevG)
          prevG.onclick = function () {
            state.offset -= state.limit;
            loadTrainingGroupsCatalog();
          };
        var nextG = document.getElementById("pageNextGroups");
        if (nextG)
          nextG.onclick = function () {
            state.offset += state.limit;
            loadTrainingGroupsCatalog();
          };
        var footerHtml =
          'Не нашли группу? <span class="link" id="linkGeneralRequestFromGroupList">Оставить заявку</span>';
        ...
        if (state.offset === 0 && !shouldSkipCatalogConnectionHeavyPrefetch()) {
          ...
          scheduleCatalogIdlePrefetch(function () {
            ...
            getJson("/training-groups", nextPageParams)
              .then(function (nextData) {
                if (mr !== catalogListReqId) return;
                var nextItems =
                  nextData && nextData.items ? nextData.items : [];
                warmupImageUrls(
                  nextItems
                    .slice(0, 6)
                    .map(function (g) {
    ...
    request
      .then(function (data) {
        ...
        if (!items.length) {
          ...
          if (
            state.collectiveSlug &&
            isStudioRosterBrand(state.collectiveBrand) &&
            !hasSlotFilters
          ) {
            revealCatalogListContent(
              buildStudioRosterEmptyHtml(state.collectiveBrand),
            );
            document.getElementById("pagination").innerHTML = "";
            document.getElementById("trainerListFooter").innerHTML = "";
            document.getElementById("trainerListFooter").style.display = "none";
          ...
          var emptyHint = hasSlotFilters
            ...
            : state.cityId && state.serviceId
              ...
              : "";
          revealCatalogListContent(
            '<div class="empty">Тренеров по вашему запросу пока нет.' +
              emptyHint +
              "</div>" +
              '<div class="empty-state-request">' +
        ...
        if (myReq !== catalogListReqId) return;
        document
          .querySelectorAll("link[data-tcb-catalog-preload]")
          .forEach(function (el) {
            el.remove();
          });
        ...
        var html = items
          .map(function (t, idx) {
            ...
            if (services.length) {
              var s0 = services[0];
              metaBottom.push(
                (s0.service_name || "Услуга") +
                  ": " +
                  formatCatalogServicePrice(s0),
              );
            }
            ...
            if (metaBottom.length) {
              infoHtml +=
                '<div class="meta">' + metaBottom.join(" · ") + "</div>";
            }
            infoHtml += "</div>";
            var groupBadgeHtml = "";
            if (
              typeof t.open_groups_count === "number" &&
              t.open_groups_count > 0
            ) {
              groupBadgeHtml =
                '<div class="catalog-trainer-group-badge">Набор в группу</div>';
            }

            var photoHtml = src
              ? '<div class="trainer-card-photo-wrap"><div class="trainer-card-photo-placeholder"></div><img src="' +
                src +
                '" alt="" loading="' +
                (isPriority ? "eager" : "lazy") +
                '" fetchpriority="' +
                (isPriority ? "high" : "auto") +
                '" decoding="async" class="trainer-card-img"' +
                (proxyFallback
                  ? ' data-fallback="' +
                    proxyFallback.replace(/"/g, "&quot;") +
                    '"'
            ...
            return (
              '<button type="button" class="trainer-list-card" data-index="' +
              idx +
              '">' +
              photoHtml +
              '<div class="trainer-list-card__main">' +
              infoHtml +
              groupBadgeHtml +
              "</div>" +
              '<span class="arrow">→</span></button>'
        ...
        revealCatalogListContent(html);
        document
          .querySelectorAll("#trainerList .trainer-card-img")
          .forEach(function (img) {
            function showLoaded() {
              img.classList.add("loaded");
        ...
        document
          .querySelectorAll("#trainerList .trainer-list-card")
          .forEach(function (btn) {
            btn.onclick = function () {
        ...
        var footerHtml =
          'Не нашли подходящего? <span class="link" id="linkGeneralRequestFromList">Оставить заявку</span>';
        document.getElementById("trainerListFooter").innerHTML = footerHtml;
        document.getElementById("trainerListFooter").style.display = "block";
        ...
        if (state.offset === 0 && !shouldSkipCatalogConnectionHeavyPrefetch()) {
          ...
          scheduleCatalogIdlePrefetch(function () {
            ...
            getJson("/trainers", nextPageParams)
              .then(function (nextData) {
                if (mr !== catalogListReqId) return;
                var nextItems =
                  nextData && nextData.items ? nextData.items : [];
                warmupImageUrls(
                  nextItems.slice(0, 6).map(function (t) {
                    return (
                      photoSrcList(t.photos && t.photos[0]) ||
                      photoSrcDetail(t.photos && t.photos[0])
                    );
                  }),
                );
              })
  ...
  function slotGroupSpotsPillHtml(s) {
    ...
    if (cap > 1 && sl != null && !isNaN(sl) && sl > 0) {
      ...
      return (
        '<span class="slot-spots-pill" aria-label="' +
        label.replace(/"/g, "&quot;") +
        '">' +
        label +
        "</span>"
  ...
  function loadSlotsForTrainer(trainerId, trainerFromList) {
    ...
    if (state.serviceId != null)
      q += "&service_id=" + encodeURIComponent(state.serviceId);
    ...
    var headers = {};
    if (initData) headers["X-Telegram-Init-Data"] = initData;
    return fetch("/api/webapp/client/slots?" + q, { headers: headers }).then(
      function (r) {
        return r.json().then(function (data) {
          if (!r.ok) throw new Error(data.detail || r.statusText);
          state.slotsEmptyOnFilterHint =
            data && data.empty_on_filter ? data.empty_on_filter_hint || "" : "";
          return data;
        });
      },
    );
  ...
  function trainerSlotsArenaIdsEqual(a, b) {
    var left = (a || []).slice().sort(function (x, y) {
      return x - y;
    });
    var right = (b || []).slice().sort(function (x, y) {
      return x - y;
    });
  ...
  function pickDefaultTrainerSlotsArenaIds(trainer, slots) {
    ...
    var primary =
      trainer.primary_arena_id != null ? Number(trainer.primary_arena_id) : NaN;
  ...
  function initTrainerSlotsArenaFilter(trainer, opts) {
    opts = opts || {};
    var tarenas = (trainer && trainer.arena_ids ? trainer.arena_ids : [])
      .map(Number)
      .filter(function (n) {
        return !isNaN(n);
      });
    ...
    var explicit = Array.isArray(opts.explicitArenaIds)
      ? opts.explicitArenaIds
      : null;
    if (explicit && explicit.length) {
      var picked = explicit
        .filter(function (id) {
          return tarenas.indexOf(Number(id)) >= 0;
        })
        .map(Number);
  ...
  function loadTrainerDetailSlots(trainer) {
    return loadSlotsForTrainer(trainer.id, trainer).then(function (data) {
      ...
      if (!state.trainerSlotsArenaFilterExplicit) {
        ...
        if (
          picked.length &&
          !trainerSlotsArenaIdsEqual(picked, state.trainerSlotsArenaIds)
        ) {
          state.trainerSlotsArenaIds = picked;
          return loadSlotsForTrainer(trainer.id, trainer).then(
            function (data2) {
  ...
  function paintTrainerArenaSlotFilterBar(t) {
    ...
    el.style.display = "block";
    var allMode =
      !state.trainerSlotsArenaIds || state.trainerSlotsArenaIds.length === 0;
    var html = "";
    html += '<div class="trainer-detail-arena-filter">';
    html +=
      '<div class="trainer-detail-arena-filter-title">Показать слоты</div>';
    html +=
      '<div class="trainer-detail-arena-filter-chips" role="group" aria-label="Площадки">';
    html +=
  ...
  function refetchTrainerSlotsForCard() {
    ...
    document.getElementById("trainerDetailSlots").innerHTML =
      buildTrainerDetailSlotsSkeletonHtml();
    ...
    loadSlotsForTrainer(tt.id, tt)
      ...
      .catch(function () {
        ...
        appendContactTrainerButton(errActions, tt, {
          primary: true,
          label: "Написать тренеру",
        });
        errActions.innerHTML +=
          '<button type="button" class="btn-secondary btn-block" data-action="leave-request">Оставить заявку</button>';
  ...
  function paintTrainerSlotsAndActionsSection(t, data) {
    ...
    if (slots.length === 0) {
      var emptyHint =
        (data && data.empty_on_filter_hint) ||
        state.slotsEmptyOnFilterHint ||
        "";
      slotsHtml +=
        '<div class="slots-empty">Сейчас нет свободных слотов.</div>';
      slotsHtml +=
        '<div class="slots-empty slots-empty-hint">' +
        escapeHtml(
          emptyHint || "Оставьте заявку — тренер предложит удобное время.",
        ) +
        "</div>";
    } else {
      ...
      for (var i = 0; i < showCount; i++) {
        var s = slots[i];
        var place =
          window.TrainerArenaChips &&
          typeof window.TrainerArenaChips.formatSlotPlaceCaption === "function"
        ...
        slotsHtml +=
          ...
          "</span>" +
          (place
            ? '<span class="slot-row-place">' + escapeHtml(place) + "</span>"
            : "") +
    ...
    slotsEl
      .querySelectorAll(".slot-row[data-slot-index]")
      .forEach(function (btn) {
        btn.onclick = function () {
    ...
    if (slots.length > 0) {
      actionsEl.innerHTML +=
        '<button type="button" class="btn-primary btn-block" id="btnBookFromDetail">Записаться</button>';
    }
    appendContactTrainerButton(actionsEl, t, {
      primary: slots.length === 0,
      label: "Написать тренеру",
    });
    actionsEl.innerHTML +=
      '<button type="button" class="btn-secondary btn-block" data-action="leave-request">Оставить заявку</button>';
    if (t.has_pass_products || t.has_certificate_products) {
      actionsEl.innerHTML +=
        '<button type="button" class="btn-secondary btn-block" data-action="buy-pass">Абонементы/Сертификаты</button>';
    }
  ...
  function wireTrainerArenaChips(root) {
    if (!root) return;
    root
      .querySelectorAll("a.trainer-arena-chip[href]")
      .forEach(function (link) {
        link.addEventListener("click", function (ev) {
          ...
          if (
            window.ClientShell &&
            typeof window.ClientShell.navigate === "function"
          ) {
  ...
  function buildTrainerArenasBlock(t) {
    ...
    if (chipsApi && typeof chipsApi.buildTrainerArenaChips === "function") {
      return chipsApi.renderTrainerArenaChipsHtml(
        chipsApi.buildTrainerArenaChips(t),
      );
    }
    var ids = t.arena_ids || [];
    var names = t.arena_names || [];
    ...
    if (n <= 1) {
      return (
        ...
        escapeHtml(names[0] && names[0].trim() ? names[0] : "—") +
        "</div>"
      );
    }
    ...
    html +=
      '<span class="trainer-detail-arena-primary-name">' +
    ...
    if (others.length) {
      ...
      others.forEach(function (nm) {
        html +=
          '<div class="trainer-detail-arena-secondary">' +
          escapeHtml(nm) +
          "</div>";
      });
      html += "</div>";
    ...
    html +=
      '<p class="trainer-detail-arena-hint">Запись доступна на те площадки, где у тренера есть свободные слоты. Ниже можно отфильтровать по арене.</p>';
    html += "</div>";
    return html;
  }
  ...
  function renderTrainerDetail() {
    ...
    var ratingStr =
      p.rating_avg != null && (p.rating_count || 0) > 0
        ? p.rating_avg.toFixed(1) + " ★ (" + p.rating_count + ")"
        : "—";
    ...
    if (chipsHtml) {
      arenaStatClass +=
        " trainer-detail-stat--arenas trainer-detail-stat--chips";
      ...
      arenaInnerHtml =
        '<div class="trainer-detail-stat-value">' +
        escapeHtml(t.arena_names && t.arena_names[0] ? t.arena_names[0] : "—") +
        "</div>";
    } else {
      arenaInnerHtml = '<div class="trainer-detail-stat-value">—</div>';
    }
    var services = t.services || [];
    var servicesStr = services.length
      ? services
          .map(function (s) {
            return (
              (s.service_name || "—") + ": " + formatCatalogServicePrice(s)
            );
          })
          .join(", ")
      : "—";
    ...
    if (listSrc || fullSrc) {
      ...
      if (listSrc && fullSrc && listSrc !== fullSrc) {
        html +=
          '<img src="' +
          listSrc +
          '" alt="" loading="eager" fetchpriority="high" decoding="async" class="trainer-detail-photo-base"';
        if (fbList) html += ' data-fallback-base="' + escapeAttr(fbList) + '"';
        html += ">";
        html +=
          '<img src="' +
          fullSrc +
          '" alt="" loading="eager" fetchpriority="high" decoding="async" class="trainer-detail-photo-full"';
        ...
        html +=
          '<img src="' +
          oneSrc +
          '" alt="" loading="eager" fetchpriority="high" decoding="async" class="trainer-detail-photo trainer-detail-photo-single"';
        if (fbFull || fbList)
          html += ' data-fallback="' + escapeAttr(fbFull || fbList) + '"';
        html += ">";
      }
      html += "</div>";
    }
    ...
    if (ratingStr !== "—") {
      html +=
        '<div class="trainer-detail-rating" style="cursor: pointer;" onclick="showTrainerReviews(' +
        t.id +
        ')">⭐ ' +
        ratingStr +
        "</div>";
    ...
    if (statsParts.length) {
      html +=
        '<div class="trainer-detail-stats">' + statsParts.join("") + "</div>";
    }
    ...
    if (services.length > 0) {
      ...
      if (state.serviceId != null) {
        ...
        for (sj = 0; sj < services.length; sj++) {
          var sx = services[sj];
          if (sx.service_id == null || Number(sx.service_id) !== selSvcId)
            continue;
          var rawDesc = sx.description && String(sx.description).trim();
          if (rawDesc)
            selectedServiceDescEscaped = escapeHtml(rawDesc).replace(
              /\n/g,
              "<br>",
            );
      ...
      if (selectedServiceDescEscaped) {
        html +=
          '<div class="trainer-detail-service-callout" role="region" aria-label="Описание услуги">';
        html +=
          '<div class="trainer-detail-service-callout-kicker">Описание услуги</div>';
        html +=
          '<div class="trainer-detail-service-callout-body">' +
          selectedServiceDescEscaped +
          "</div>";
        html += "</div>";
      }
      ...
      services.forEach(function (s) {
        ...
        if (isClickable) {
          html +=
            '<button type="button" class="trainer-detail-service trainer-detail-service--clickable" data-catalog-service-select="' +
            sid +
            '">';
        } else {
          html +=
        ...
        html +=
          '<span class="trainer-detail-service-name">' +
          escapeHtml(serviceName) +
          "</span>";
        html +=
          '<span class="trainer-detail-service-price">' + priceText + "</span>";
        html += "</div>";
    ...
    var profileEduDetail =
      p.education && typeof p.education === "string" && p.education.trim()
        ? p.education.trim()
        : "";
    ...
    var htmlAbout = buildTrainerAboutSectionHtml(
      desc,
      eduRows,
      profileEduDetail,
      profileExtraForDetail,
    ...
    if (!canBook) {
      ...
      if (ph)
        phoneLine +=
          '<div class="slots-empty slots-contact"><strong>Телефон:</strong> ' +
          phoneCallHtml(ph) +
          "</div>";
      if (ct)
        phoneLine +=
          '<div class="slots-empty slots-contact"><strong>Контакты:</strong> ' +
          escapeHtml(ct) +
          "</div>";
    ...
    loadTrainerDetailSlots(t)
      ...
      .catch(function () {
        ...
        var errActions = document.getElementById("trainerDetailActions");
        errActions.innerHTML = "";
        appendContactTrainerButton(errActions, t, {
          primary: true,
          label: "Написать тренеру",
        });
        errActions.innerHTML +=
  ...
  function renderSlotPickList() {
    ...
    days.forEach(function (dateKey) {
      ...
      if (d.length === 3) {
        ...
        var dayName = SLOT_DAYS[dateObj.getDay()];
        var dateStr =
          day + "." + String(month + 1).padStart(2, "0") + " (" + dayName + ")";
        html +=
          '<div class="day-block"><div class="day-title">' + dateStr + "</div>";
      }
  ...
  (function wireCatalogBookingPhoneMask() {
    if (window.CrmPhoneField)
      CrmPhoneField.initAll(
        document.getElementById("screenBookingForm") || document,
      );
  })();
  ...
  document.getElementById("btnSubmitBooking").onclick = function () {
    ...
    var phCheck = window.CrmPhoneField
      ? CrmPhoneField.validate(phoneEl)
      : { ok: false, error: "Укажите номер телефона." };
    if (!phCheck.ok) {
    ...
    var askName =
      state.needsProfileName &&
      !(selectedProfile && selectedProfile.role === "guardian");
    if (askName) {
      var fn = (
        (document.getElementById("bookingFirstName") &&
          document.getElementById("bookingFirstName").value) ||
        ""
      ).trim();
      if (!fn) {
        alert("Укажите имя");
    ...
    btn.disabled = true;
    var initData = tg ? tg.initData : "";
    if (!state.catalogBookingIdempotencyKey)
      assignCatalogBookingIdempotencyKeyForSlot();
    var payloadBody = window.BookingClient
      ? window.BookingClient.buildBookingPayload({
          ...
          phone: phone,
          comment:
            (document.getElementById("bookingComment").value || "").trim() ||
            null,
    ...
    if (!window.BookingClient) {
      if (askName) {
        payloadBody.first_name = (
          document.getElementById("bookingFirstName").value || ""
        ).trim();
        var ln = (
          document.getElementById("bookingLastName").value || ""
        ).trim();
      ...
      if (state.catalogBookingPriceVariantId != null) {
        payloadBody.service_price_variant_id =
          state.catalogBookingPriceVariantId;
      }
    }
      ...
      : fetch("/api/webapp/client/booking", {
          ...
          headers: (function () {
            ...
            if (actingProfileId != null)
              h["X-Profile-Id"] = String(actingProfileId);
            return h;
          })(),
    ...
    submitFn
      .then(function (out) {
        ...
        if (data == null) {
          if (r.ok) {
            ...
            alert(
              "Ошибка сервера (" +
                r.status +
                "). Проверьте «Мои записи» в боте перед повтором.",
            );
          }
          return;
        }
        if (r.ok && data.success) {
          ...
          clearCatalogSessionStorageCache();
          persistCatalogFilters(
            state.selectedTrainer && state.selectedTrainer.id,
          );
          if (window.BookingClient) {
  ...
  var btnSuccessBookingsCatalog = document.getElementById(
    "btnSuccessBookingsCatalog",
  );
  ...
  document.getElementById("backToCity").onclick = function () {
    showScreen("screenCity");
  };
  document.getElementById("backToService").onclick = function () {
    showScreen("screenService");
  };
  ...
  function updateRequestFormContext() {
    ...
    if (state.requestForTrainer) {
      ...
      ctxEl.innerHTML =
        '<div class="context-line">Заявка придёт <strong>только этому тренеру</strong>.</div>';
    } else {
      titleEl.textContent = "Общая заявка";
      ctxEl.innerHTML =
        '<div class="context-line">Любой тренер по вашему городу и услуге увидит заявку.</div>';
    }
  }
  ...
  document
    .getElementById("trainerDetailActions")
    .addEventListener("click", function (e) {
      ...
      if (action && action.dataset.action === "buy-pass") {
        ...
        if (state.cityId != null)
          q += "&city_id=" + encodeURIComponent(state.cityId);
        if (state.serviceId != null)
          q += "&service_id=" + encodeURIComponent(state.serviceId);
        if (state.arenaId != null)
          q += "&arena_id=" + encodeURIComponent(state.arenaId);
        if (state.offset != null)
          q += "&offset=" + encodeURIComponent(state.offset);
  ...
  function submitRequestFromForm(comment) {
    ...
    if (state.requestForTrainer && state.requestForTrainer.id)
      body.trainer_id = state.requestForTrainer.id;
    var commentStr =
      comment != null && String(comment).trim() ? String(comment).trim() : null;
    if (commentStr) body.comment = commentStr;
    if (state.needsProfileName) {
      var rfn = (
        (document.getElementById("requestFirstName") &&
      ...
      body.first_name = rfn;
      var rln = (
        (document.getElementById("requestLastName") &&
      ...
      .then(function (r) {
        return r.json().then(function (data) {
          if (r.ok && data.success) {
            ...
            clearCatalogSessionStorageCache();
            var isPersonal = !!(
              state.requestForTrainer && state.requestForTrainer.id
            );
            var trainerName =
              isPersonal && state.requestForTrainer.name
                ? state.requestForTrainer.name
                : "";
            document.getElementById("successText").innerHTML = isPersonal
              ? "✅ <b>Заявка отправлена</b> " +
                (trainerName ? trainerName : "тренеру") +
                ".<br><br>Когда тренер ответит — напишем вам в боте."
  ...
  document.getElementById("tabCatalog").onclick = function () {
    switchTab("catalog");
  };
  document.getElementById("tabMyTrainer").onclick = function () {
    switchTab("my_trainer");
  };
  ...
  document.getElementById("tabMyTrainerFromDetail").onclick = function () {
    /* already on my trainer card */
  };
  ...
  (function bootstrapCatalogUiFast() {
    var shell = window.ClientShell;
    var warm =
      shell && shell.readCatalogWarmCache ? shell.readCatalogWarmCache() : null;
    var qp = new URLSearchParams(window.location.search || "");
    ...
    if (
      home &&
      !slotBookingDeepLink &&
      !trainerDetailDeepLink &&
      !bookActionDeepLink
    )
      home.classList.add("catalog-home--booting");
  ...
  Promise.all([loadTrainerEdges(), getClientSession()])
    .then(function (results) {
      ...
      var qp = new URLSearchParams(window.location.search || "");
      var explicitTrainerArenaIdsFromUrl =
        parseExplicitTrainerArenaIdsFromQuery(qp);
      bootstrapCollectiveFromQuery(qp).finally(function () {
        applySessionToState(session);
        if (
          state.collectiveBrand &&
          state.collectiveBrand.default_city_id &&
          !qp.get("city_id")
        ) {
        ...
        var deepTidParsed =
          rawDeepTid != null && rawDeepTid !== ""
            ? parseInt(rawDeepTid, 10)
            : NaN;
        var deepTrainerFromUrl =
          !isNaN(deepTidParsed) && deepTidParsed > 0 ? deepTidParsed : null;
        ...
        if (returnCtx) {
          if (returnCtx.city_id != null)
            state.cityId = parseInt(returnCtx.city_id, 10) || null;
          if (returnCtx.service_id != null)
            state.serviceId = parseInt(returnCtx.service_id, 10) || null;
          ...
          if (returnCtx.arena_ids != null && returnCtx.arena_ids !== "") {
            var deepIds = returnCtx.arena_ids
              .split(",")
              .map(function (s) {
                return parseInt(s, 10);
              })
            ...
            var deepOne =
              returnCtx.arena_id === ""
                ? null
                : parseInt(returnCtx.arena_id, 10) || null;
            if (deepOne) setArenaSelection([deepOne], ["Арена"]);
            else clearArenaSelection();
          }
          if (returnCtx.offset != null)
            state.offset = Math.max(0, parseInt(returnCtx.offset, 10) || 0);
          if (returnCtx.slot_id != null && returnCtx.slot_id !== "") {
            var deepSlot = parseInt(returnCtx.slot_id, 10);
            state.pendingDeepLinkSlotId =
              !isNaN(deepSlot) && deepSlot > 0 ? deepSlot : null;
          }
          var dVariant = returnCtx.service_price_variant_id;
          if (dVariant != null && dVariant !== "") {
            var dv = parseInt(dVariant, 10);
            if (!isNaN(dv) && dv > 0)
              state.bookingContextServicePriceVariantId = dv;
          }
        }
        function showInitialScreen() {
          if (
            catalogSummaryHydrated &&
            freshSessionFingerprint === catalogHydrateFingerprint
          ) {
            activateSummaryScreen();
            switchTab(state.activeTab || "catalog");
            refreshCatalogOfferTrainerCount();
        ...
        if (returnCtx && returnCtx.trainer_id) {
          ...
          openTrainerDeepLinkTrainerCard(
            returnCtx,
            explicitTrainerArenaIdsFromUrl,
            function () {
              if (state.trainerId) {
                ...
                loadTrainerById(state.trainerId)
                  .then(function (t2) {
                    if (t2) {
                      state.trainerId =
                        t2.id != null ? Number(t2.id) : state.trainerId;
                      state.selectedTrainer = t2;
                      state.trainerName = trainerName(t2);
                      reconcileCatalogServiceWithTrainerAsync(t2).then(
                        function () {
                          initTrainerSlotsArenaFilter(t2, {
                            explicitArenaIds: explicitTrainerArenaIdsFromUrl,
                          });
                          renderTrainerDetail();
                          showScreen("screenTrainerDetail");
              ...
              if (returnCtx && returnCtx.trainer_id) {
                showToast(
                  "Не удалось открыть карточку тренера. Попробуйте ещё раз из ссылки в чате.",
                );
              }
              showInitialScreen();
            },
  ...
  document.addEventListener("visibilitychange", function () {
    ...
    if (catalogVisibilityDebounceTimer)
      clearTimeout(catalogVisibilityDebounceTimer);
  ...
  document.addEventListener("click", function (e) {
    var svcPick =
      e.target &&
      e.target.closest &&
      e.target.closest("#trainerDetailTop [data-catalog-service-select]");
    if (svcPick) {
    ...
    var actionBtn =
      ...
      e.target.closest &&
      e.target.closest(
        "#summaryRows .catalog-filter-tile, #summaryRows .summary-row-clickable, #tabCatalog, #tabMyTrainer",
      );
    if (actionBtn) {
  ...
  window.showTrainerReviews = function (trainerId) {
    ...
    fetch(
      "/api/public/trainers/" + encodeURIComponent(trainerId) + "/reviews",
      { cache: "no-store" },
    )
      .then(function (res) {
        return res.json().then(function (data) {
      ...
      .then(function (data) {
        ...
        if (reviews.length === 0) {
          modalBody.innerHTML =
            '<div class="reviews-empty">У этого тренера пока нет отзывов</div>';
          return;
        }
      ...
      .catch(function () {
        modalBody.innerHTML =
          '<div class="reviews-empty">Не удалось загрузить отзывы</div>';
      });
  };
