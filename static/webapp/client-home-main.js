(function () {
  ...
  if (tg) {
    ...
    if (typeof window.__applyClientHubTheme === "function")
      window.__applyClientHubTheme();
    try {
  ...
  var ICONS = {
    ...
    ticket:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2Z"/><path d="M13 5v2M13 11v2M13 17v2"/></svg>',
    ...
    bookmark:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m19 21-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"/></svg>',
    repeat:
    ...
    chart:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 20V10"/><path d="M10 20V4"/><path d="M16 20v-6"/><path d="M22 20V14"/></svg>',
  };
  ...
  function renderStreakRibbon(activity) {
    ...
    if (!el) return;
    var sw =
      activity && activity.streak_weeks != null
        ? Number(activity.streak_weeks)
        : 0;
    if (sw < 1) {
  ...
  function hubSessionNowPillHtml(b) {
    ...
    return '<span class="hub-session-now-pill" role="status" aria-label="Текущее занятие">сейчас</span>';
  }
  ...
  function openTelegramDm(username, telegramId, trainerId) {
    ...
    if (trainerId != null && String(trainerId).trim() !== "" && un) {
      var path =
        "/r/tg/" + encodeURIComponent(String(trainerId)) + "?src=client_app";
  ...
  function setHeroLayout(mode) {
    ...
    if (hero)
      hero.classList.toggle("hub-hero--booking-led", mode === "booking-led");
    document.body.classList.toggle(
      "hub-body--booking-led",
      mode === "booking-led",
    );
  }
  ...
  function renderNextBookingCard(item) {
    ...
    var statusLabel = isPending
      ? "Ожидает подтверждения ⏳"
      : "Подтверждено ✅";
    var statusClass = isPending
      ? "hub-next-card-status-line--pending"
      : "hub-next-card-status-line--confirmed";
    ...
    var placeHtml = place
      ? '<div class="hub-next-card-place-line">' +
        ICONS.pin +
        "<span>" +
        esc(place) +
        "</span></div>"
    ...
    if (canWriteTrainer(b)) {
      toolbarParts.push(
        '<button type="button" class="hub-next-card-tool" data-hub-dm="next"' +
          ' data-dm-un="' +
          esc((b.trainer_telegram_username || "").replace(/^@/, "")) +
          '"' +
          ' data-dm-tid="' +
          esc(
            b.trainer_telegram_id != null ? String(b.trainer_telegram_id) : "",
          ) +
          '">' +
          ICONS.msg +
    ...
    var cardMod = isPending
      ? "hub-next-card--pending"
      : "hub-next-card--confirmed";
    var html =
      '<div class="hub-next-card ' +
      cardMod +
      '" id="nextCard" data-bid="' +
      esc(String(b.id)) +
      '" tabindex="0" role="button"' +
      ' aria-label="Ближайшая запись, ' +
      esc(dateLabel) +
      " в " +
      esc(time) +
      '">' +
      ...
      '<span class="hub-next-card-status-line ' +
      statusClass +
      '">' +
      ...
      '<div class="hub-next-card-time">' +
      esc(time) +
      "</div>" +
      hubSessionNowPillHtml(b) +
      "</div>" +
      '<div class="hub-next-card-date">' +
      esc(dateLabel) +
      " · " +
      esc(String(dur)) +
      " мин</div>" +
      '<div class="hub-next-card-who">' +
      '<div class="hub-next-card-trainer-avatar" aria-hidden="true">' +
      esc(initials(trainerName)) +
      "</div>" +
      '<div class="hub-next-card-who-text">' +
      '<div class="hub-next-card-trainer-name">' +
      esc(trainerName) +
      "</div>" +
      placeHtml +
      "</div>" +
    ...
    card.addEventListener("click", function (ev) {
      var dmBtn =
        ev.target &&
        ev.target.closest &&
        ev.target.closest('[data-hub-dm="next"]');
      if (dmBtn) {
        ev.stopPropagation();
        if (
          window.ClientShell &&
      ...
      var shareNext =
        ev.target &&
        ev.target.closest &&
        ev.target.closest('[data-hub-action="share-trainer"]');
      if (shareNext) {
        ev.stopPropagation();
        if (
          window.ClientShell &&
  ...
  function catalogPrimaryServiceQuery() {
    if (primaryCatalogServiceId == null || primaryCatalogServiceId === "")
      return "";
  ...
  function navigateToBookAgain(trainerId, serviceId) {
    var svc =
      serviceId != null && serviceId !== ""
        ? serviceId
        : hubRepeatBookServiceId(trainerId);
    navigateTo(
      buildBookPathFromHubContext(trainerId, {
        serviceId: svc,
  ...
  function navigateToPrimaryTrainerBook() {
    ...
    navigateToBookAgain(
      selectedTrainerId,
      hubRepeatBookServiceId(selectedTrainerId),
    );
  }
  ...
  function navigateToHubBookLikePrimaryFab(nextBooking) {
    if (
      hubPrimaryTrainerCanBook &&
      selectedTrainerId != null &&
      String(selectedTrainerId).trim() !== ""
    ) {
  ...
  function orderedRebookPills(nextBooking, targets) {
    ...
    if (!isNaN(nextTid) && nextTid > 0 && list.length >= 2) {
      ...
      for (var i = 0; i < list.length; i++) {
        if (Number(list[i].trainer_id) === nextTid) {
          idx = i;
          break;
        }
      }
  ...
  function finishHubInitialLoading() {
    ...
    var wait = Math.max(
      0,
      HUB_SKELETON_MIN_MS - (Date.now() - hubLoadStartedAt),
    );
    setTimeout(function () {
      if (document.body.classList.contains("hub-body--revealed")) return;
  ...
  function renderMyTrainerCard(
    ...
    if (trainerId != null) {
      actionBtns +=
        ...
        ' data-share-tid="' +
        esc(String(trainerId)) +
        '"' +
        ' data-share-context="my_trainer"' +
        ' aria-label="Поделиться тренером">' +
        ICONS.share +
        "</button>";
    }
    ...
    var subText = onlineBook
      ...
      : hasDm
        ? "Онлайн-запись недоступна — напишите тренеру"
        : "Открыть профиль в каталоге";
    ...
    var avatarHtml = src
      ? '<div class="hub-trainer-avatar hub-trainer-avatar--photo"><img src="' +
        esc(src) +
        '" alt="" loading="lazy"/></div>'
      : '<div class="hub-trainer-avatar">' +
    ...
    block.innerHTML =
      ...
      '<div class="hub-trainer-name">' +
      esc(displayName) +
      "</div>" +
      '<div class="hub-trainer-sub">' +
      esc(subText) +
      "</div>" +
      "</div>" +
      actionsHtml +
    ...
    card.addEventListener("click", function (ev) {
      var dmBtn =
        ev.target &&
        ev.target.closest &&
        ev.target.closest('[data-hub-dm="trainer"]');
      if (dmBtn) {
        ev.stopPropagation();
        openTelegramDm(
          dmBtn.getAttribute("data-dm-un"),
  ...
  function renderNewClientHeroActions() {
    var el = document.getElementById("hubHeroActions");
    if (!el) return;
    var searchIcon = (ICONS.search || "").replace(
      "<svg ",
      '<svg class="hub-hero-actions__icon" ',
    );
    el.innerHTML =
  ...
  function mountQuickStripPills(pills) {
    ...
    strip.innerHTML = pills
      .map(function (p, i) {
        ...
        return (
          '<button type="button" class="hub-quick-pill' +
          primaryClass +
          '" data-pill-idx="' +
          i +
          '">' +
    ...
    return [
      {
        ...
        primary: true,
        action: function () {
          navigateToHubBookLikePrimaryFab(nextBooking);
        },
      },
    ];
  }
  ...
  function pluralRuHub(n, one, few, many) {
    if (ruText && typeof ruText.pluralRu === "function")
      return ruText.pluralRu(n, one, few, many);
  ...
  function buildPrimarySlotsHtml(slots) {
    ...
    var items = slots
      ...
      .map(function (s) {
        ...
        var place = ((s && (s.arena_name || s.arena_city_name)) || "")
          .toString()
          .trim();
        var placeHtml = place
          ? '<span class="hub-primary-panel__slot-place">' +
            ICONS.pin +
            esc(place) +
            "</span>"
          : "";
        return (
          '<button type="button" class="hub-primary-panel__slot" data-slot-id="' +
          esc(String(s.id || "")) +
          '"' +
          (s && s.arena_id != null
            ? ' data-arena-id="' + esc(String(s.arena_id)) + '"'
            : "") +
          (s && s.service_id != null
            ? ' data-service-id="' + esc(String(s.service_id)) + '"'
            : "") +
          ">" +
          '<span class="hub-primary-panel__slot-day">' +
          esc(dayLabel) +
          "</span>" +
          '<span class="hub-primary-panel__slot-time">' +
          esc(t) +
          "</span>" +
          placeHtml +
          "</button>"
    ...
    return (
      ...
      '<div class="hub-primary-panel__slots-scroller">' +
      items +
      "</div>" +
      "</div>"
    );
  ...
  function passProductNameForHubSubline(passInfo, total) {
    ...
    if (total > 0) {
      ...
      var sizeRe = new RegExp(
        "^" + total + "\\s*(занятие|занятия|занятий)$",
        "i",
      );
  ...
  function buildPrimaryPassHtml(passInfo, opts) {
    ...
    var expiry = passInfo.expires_at
      ? formatHistoryDate(passInfo.expires_at)
      : "";
    var subParts = [];
    ...
    var subHtml = subParts.length
      ? '<span class="hub-primary-panel__pass-sub">' +
        subParts.join(" · ") +
        "</span>"
      : "";
    var autoNoteHtml = opts.autoDebitNote
      ...
      : "";
    var passMod = opts.autoDebitNote
      ? " hub-primary-panel__pass--auto-debit"
      : "";
    var titleHtml;
    var ariaBalance;
    if (total > 0) {
      var totalLabel = ruText.genitiveCountRu(total, "занятия", "занятий");
      titleHtml =
        '<span class="hub-primary-panel__pass-kicker">осталось</span>' +
        '<span class="hub-primary-panel__pass-title">из ' +
        esc(totalLabel) +
        "</span>";
      ariaBalance =
        "Осталось " + remaining + " из " + totalLabel + " по абонементу";
    } else {
      var word = pluralRuHub(remaining, "занятие", "занятия", "занятий");
      titleHtml =
        '<span class="hub-primary-panel__pass-kicker">осталось</span>' +
        '<span class="hub-primary-panel__pass-title">' +
        word +
        " по абонементу</span>";
      ariaBalance = "Осталось " + remaining + " " + word + " по абонементу";
    }
    return (
      '<button type="button" class="hub-primary-panel__pass' +
      passMod +
      '" data-hub-action="open-pass"' +
      ' aria-label="' +
      esc(ariaBalance) +
      '">' +
      '<span class="hub-primary-panel__pass-num" aria-hidden="true">' +
  ...
  function buildPrimaryHistoryHtml(history) {
    ...
    var lastStr = history.last_completed_at
      ? formatHistoryDate(history.last_completed_at)
      : "";
  ...
  function renderPrimaryPassPanel(trainerId, bootstrapPasses, opts) {
    ...
    var inner = rowHead
      ? '<div class="hub-primary-panel__row hub-primary-panel__row--pass">' +
        '<div class="hub-primary-panel__row-head">' +
        esc(rowHead) +
        "</div>" +
        passHtml +
        "</div>"
    ...
    el.removeAttribute("hidden");
    var wireTid =
      trainerId != null ? trainerId : passInfo && passInfo.trainer_id;
  ...
  function renderHubSecondaryFill(hubMeta, opts) {
    ...
    renderPrimaryPassPanel(
      opts.trainerId != null ? opts.trainerId : null,
      passes,
      {
  ...
  function loadAndRenderPrimaryPanel(trainerId, history, bootstrapPasses) {
    ...
    if (_primaryPanelAbort) {
      _primaryPanelAbort.abort();
    }
    var ctl =
    ...
    var slotsPromise = fetch(slotsUrl, {
      headers: headersJson(),
      signal: ctl ? ctl.signal : undefined,
    })
      .then(function (r) {
        return r.ok ? r.json() : { slots: [] };
      })
      .catch(function (e) {
        return e && e.name === "AbortError" ? null : { slots: [] };
      });
  ...
  function wirePrimaryPanelClicks(el, trainerId) {
    ...
    el.addEventListener("click", function (ev) {
      var slotBtn =
        ev.target &&
        ev.target.closest &&
        ev.target.closest(".hub-primary-panel__slot");
      if (slotBtn) {
        ev.stopPropagation();
        navigateTo(
          buildBookPathFromHubContext(trainerId, {
      ...
      var passBtn =
        ev.target &&
        ev.target.closest &&
        ev.target.closest('[data-hub-action="open-pass"]');
      if (passBtn) {
        ev.stopPropagation();
  ...
  function wireDiscoveryCardPhotos(root) {
    if (!root) return;
    root
      .querySelectorAll(".hub-discovery__avatar--photo img")
  ...
  function buildDiscoveryCardHtml(t) {
    ...
    var arenaHtml = arena
      ? '<div class="hub-discovery__city">' + ICONS.pin + esc(arena) + "</div>"
      : "";
    var ratingHtml = "";
    if (p && p.rating_avg != null && (p.rating_count || 0) > 0) {
      ratingHtml =
        ...
        ' <span class="hub-discovery__rating-count">(' +
    ...
    return (
      '<button type="button" class="hub-discovery__card" data-tid="' +
      tidStr +
      '">' +
      avatarHtml +
      '<div class="hub-discovery__body">' +
      '<div class="hub-discovery__name">' +
      esc(name) +
      "</div>" +
      ratingHtml +
      arenaHtml +
      ...
      .then(function (payload) {
        ...
        el.querySelectorAll(".hub-discovery__card").forEach(function (btn) {
          btn.addEventListener("click", function () {
            ...
            if (tid)
              navigateTo("catalog?trainer_id=" + encodeURIComponent(tid));
          });
        });
      });
  }
    ...
    bblock.addEventListener(
      ...
      function (ev) {
        var dmBtn =
          ev.target &&
          ev.target.closest &&
          ev.target.closest('[data-hub-dm="list"]');
        if (dmBtn) {
          ev.preventDefault();
          ev.stopPropagation();
          if (
        ...
        var row =
          ev.target &&
          ev.target.closest &&
          ev.target.closest(".hub-booking-row[data-bid]");
        if (!row) return;
  ...
  function renderUpcomingList(days, heroBookingId) {
    ...
    items.forEach(function (item) {
      ...
      if (!byDate[ds]) {
        byDate[ds] = { day: item.day, bookings: [] };
        dateOrder.push(ds);
      }
    ...
    dateOrder.forEach(function (ds) {
      ...
      group.bookings.forEach(function (b) {
        ...
        var rowClass = isPending
          ? "hub-booking-row--pending"
          : "hub-booking-row--confirmed";
        var statusClass = isPending
          ? "hub-booking-status--pending"
          : "hub-booking-status--confirmed";
        ...
        if (canWriteTrainer(b)) {
          msgBtnHtml =
            ...
            ' data-dm-tid="' +
            esc(
              b.trainer_telegram_id != null
                ? String(b.trainer_telegram_id)
                : "",
            ) +
            '"' +
            ' data-trainer-id="' +
            esc(b.trainer_id != null ? String(b.trainer_id) : "") +
            '"' +
        ...
        parts.push(
          '<div class="hub-booking-row ' +
            ...
            '<div class="hub-booking-time">' +
            esc(time) +
            "</div>" +
            hubSessionNowPillHtml(b) +
            "</div>" +
            '<div class="hub-booking-dur">' +
            esc(String(dur)) +
            " мин</div>" +
            "</div>" +
            '<div class="hub-booking-divider"></div>' +
            '<div class="hub-booking-info">' +
            '<div class="hub-booking-trainer">' +
            esc(trainerName) +
            "</div>" +
            (place
              ? '<div class="hub-booking-place">' + esc(place) + "</div>"
              : "") +
            '<span class="hub-booking-status hub-booking-status--inline ' +
            statusClass +
            '">' +
            statusLabel +
            "</span>" +
            "</div>" +
            (msgBtnHtml
              ? '<div class="hub-booking-row-end">' + msgBtnHtml + "</div>"
              : "") +
            "</div>",
        );
      });
  ...
  function renderSavedTrainersStrip(cs) {
    ...
    if (cs && Array.isArray(cs.saved_trainers) && cs.saved_trainers.length) {
      ...
      cs &&
      Array.isArray(cs.saved_trainer_ids) &&
      cs.saved_trainer_ids.length
    ) {
    ...
    var chips = list
      .map(function (row) {
        ...
        var photoPart = src
          ? '<span class="hub-saved-chip-avatar"><img src="' +
            esc(src) +
            '" alt=""/></span>'
          : '<span class="hub-saved-chip-icon" aria-hidden="true">🤍</span>';
        return (
          '<button type="button" class="hub-saved-chip" data-tid="' +
          esc(String(tid)) +
          '">' +
          photoPart +
          '<span class="hub-saved-chip-label">' +
          esc(label) +
          "</span>" +
          "</button>"
        );
    ...
    if (viewBtn)
      viewBtn.addEventListener("click", function () {
        navigateTo("client-saved-trainers");
      });
    block.querySelectorAll(".hub-saved-chip").forEach(function (btn) {
      btn.addEventListener("click", function () {
        navigateTo(
          "catalog?trainer_id=" +
            encodeURIComponent(btn.getAttribute("data-tid")),
        );
      });
    });
  }
  ...
  function applyHubState(bookingDays, requestItems, hubMeta) {
    if (
      window.ClientShell &&
      typeof window.ClientShell.writeBookingsWarmCache === "function"
    ) {
    ...
    var primaryTrainerId =
      cs.primary_trainer_id != null
        ? cs.primary_trainer_id
        : cs.selected_trainer_id != null && cs.selected_trainer_id !== ""
          ? cs.selected_trainer_id
          : null;
    var savedTrainersRich = Array.isArray(cs.saved_trainers)
      ? cs.saved_trainers
      : [];
    var savedIds = Array.isArray(cs.saved_trainer_ids)
      ? cs.saved_trainer_ids
      : [];
    ...
    if (primaryTrainerId != null) {
      ...
      setHeroText(
        ...
        "Выберите удобный слот у вашего тренера",
      );
      ...
      var ptgId =
        cs.primary_trainer_telegram_id != null
          ? cs.primary_trainer_telegram_id
          : null;
      ...
      return finishHubApply(
        loadAndRenderPrimaryPanel(
          primaryTrainerId,
          cs.primary_history || null,
          hubBootstrapPasses(hubMeta),
        ),
      );
    }
    ...
    if (hasSavedBookmarks && primaryTrainerId == null) {
      resetHubHeroLayout();
      setHeroText(
        "Ваши любимые тренеры 💛",
        "Выберите, с кем хотите позаниматься",
      );
      renderSavedTrainersStrip(cs);
      return finishHubApply(
        renderHubSecondaryFill(hubMeta, {
          showDiscovery: true,
    ...
    setHeroText(
      "Добро пожаловать!",
      "Выберите, чем хотите заняться, а мы найдём лучшего тренера",
    );
  ...
  function loadAll() {
    ...
    if (!initData) {
      ...
      return loadAndRenderDiscovery().then(
        finishHubInitialLoading,
        finishHubInitialLoading,
      );
    }
    ...
    return fetch(apiUrl("/client/hub/bootstrap"), { headers: headersJson() })
      ...
      .then(function (hub) {
        ...
        if (
          window.ClientShell &&
          typeof window.ClientShell.scheduleCatalogNavigationPrefetch ===
            "function"
        ) {
      ...
      .catch(function () {
        ...
        return Promise.all([
          fetch(apiUrl("/client/bookings"), { headers: headersJson() }).then(
            jsonOrThrow,
          ),
          fetch(apiUrl("/client/requests"), { headers: headersJson() }).then(
            jsonOrThrow,
          ),
          ...
          var sess = results[2] || {};
          renderStreakRibbon(null);
          return applyHubState(results[0].days || [], results[1].items || [], {
            client_session: {
              selected_trainer_id:
                sess.trainer_id != null ? sess.trainer_id : null,
            },
          });
        });
      })
      .catch(function () {
        setStateMessage(
          "Не удалось загрузить данные. Откройте страницу из клиентского бота.",
          "error",
        );
        hideNextBookingSkeleton();
        clearNextBookingBlock();
      ...
      if (!text) {
        if (statusEl) {
          statusEl.textContent =
            "Напишите пару слов — что случилось или что хотите улучшить.";
        ...
        .then(function (o) {
          ...
          if (okSend) {
            ...
            if (
              wg &&
              wg.HapticFeedback &&
              wg.HapticFeedback.notificationOccurred
            ) {
              try {
            ...
            if (statusEl) {
              statusEl.textContent =
                detail + " Попробуйте через /guide в боте.";
              statusEl.className = "hub-support-status hub-support-status--err";
            }
          }
        })
