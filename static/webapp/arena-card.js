...
(function (global) {
  ...
  function shellNav(path) {
    if (
      global.ClientShell &&
      typeof global.ClientShell.navigate === "function"
    ) {
  ...
  function trainerName(t) {
    var p = (t && t.profile) || t || {};
    return (
      [p.first_name, p.last_name].filter(Boolean).join(" ").trim() || "Тренер"
    );
  }
  ...
  function renderHero() {
    ...
    var style = url
      ...
      : "";
    var galleryHtml = n
      ? '<span class="arena-hero__gallery">' +
    ...
    return (
      ...
      ">" +
      (hero.mode === "photo"
        ? '<span class="arena-hero__shade"></span>'
        : '<span class="arena-hero__glare"></span>') +
  ...
  function rowHtml(row) {
    ...
    if (row.nature === "lesson" && row.group) {
      extra +=
        ' data-action="group" data-trainer="' +
        esc(row.trainerId) +
        '" data-group="' +
        esc(row.group.id) +
        '"';
    ...
    if (row.openDay)
      extra += ' data-action="open-day" data-date="' + esc(row.localDate) + '"';
    return (
      "<" +
      tag +
      ' class="' +
      ...
      extra +
      ">" +
      '<span class="arena-row__t">' +
      esc(row.time || row.weekday) +
      "</span>" +
      '<span class="arena-row__m"><b>' +
      esc(row.title) +
      "</b><span>" +
      esc(row.meta || "") +
      "</span></span>" +
  ...
  function renderGroups() {
    ...
    for (i = 0; i < groups.length; i++) {
      var g = groups[i];
      var right =
        g.spots_left > 0 && ruText && typeof ruText.countRu === "function"
          ? ruText.countRu(g.spots_left, "место", "места", "мест")
          : g.spots_left > 0
            ? g.spots_left + " места"
            : "набор открыт";
      html +=
  ...
  function renderPractice() {
    ...
    var html =
      '<div class="arena-sec"><p class="arena-h">Как добраться и что есть</p>';
  ...
  function paintRowsOnly() {
    ...
    [].forEach.call(tabs.querySelectorAll("[data-day]"), function (b) {
      b.setAttribute(
        "aria-pressed",
        b.getAttribute("data-day") === state.day ? "true" : "false",
      );
    });
  }
  ...
  function openModal(kind) {
    ...
    if (kind === "claim") {
      ...
      field.innerHTML =
        '<option value="claim">Контакт оператора катка</option>';
      value.placeholder = "телефон или сайт";
    } else if (kind === "claim-schedule") {
      title.textContent = "Дополнить карточку";
      lead.textContent =
        "Время и цены массового катания — проверим по источнику.";
      ...
      lead.textContent =
        "Укажите поле и верное значение — не письмо в поддержку.";
  ...
  function submitModal(ev) {
    ...
    if (
      ...
      global.Telegram.WebApp.showAlert
    ) {
  ...
  function onRootClick(ev) {
    ...
    if (action === "trainer") {
      ...
      if (can) goBooking(tid, "book");
        ...
        var trainer = ((state.trainers && state.trainers.items) || []).filter(
          function (x) {
  ...
  function load() {
    ...
    if (!state.ref) {
      showError(
        "Не указана арена. Откройте карточку по ссылке из каталога или бота.",
      );
      return;
    }
  ...
  function bindChrome() {
    ...
    if (
      global.ClientShell &&
      typeof global.ClientShell.setForcedTab === "function"
    ) {
