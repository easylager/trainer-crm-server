...
def resolve_trainer_next_step(
    ...
    *,
    ...
    if not has_week and not has_slots:
        # 1a. Онбординг пройден, а неделя пустая — это выбор, а не недоделка.
        #     К платформе идут не только тренеры по конькам: спортивный психолог или
        #     консультант может работать по заявкам, и часы ему называть незачем.
        #     Требовать у него расписание сразу после экрана, где мы назвали его
        #     необязательным, — значит спорить с собственным обещанием, да ещё и
        #     первым, что он видит в продукте. Ведём к первому ученику: заявку
        #     он примет и без свободных окон.
        if onboarding_done:
            return {
                "key": STEP_SHARE_LINK,
                "title": "Позовите первого ученика",
                "body": (
                    "Он напишет вам сам — для этого расписание не нужно. "
                    "Время добавите, когда будете готовы."
                ),
                "cta": {"label": "Отправить ссылку", "action": ACTION_SHARE_LINK},
                "secondary": {
                    "label": "Добавить расписание",
                    "action": ACTION_OPEN_ONBOARDING,
                },
            }
        # 1b. Онбординг ещё не проходили — вот здесь настройка действительно первый шаг.
        return {
            "key": STEP_SETUP_WEEK,
            ...
            "body": "Две минуты: кто вы, что проводите и где. Расписание — по желанию.",
