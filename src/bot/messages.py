...
- Trainer bot: «ты», коротко и по делу; пуши: заголовок + суть + CTA «Обзор» (кнопка слева от поля ввода).
...
TRAINER_AFTER_LINK_HERO = (
    ...
    "Откройте <b>«Обзор»</b> ниже — там «Первые шаги»."
)
TRAINER_AFTER_LINK_STEP_BOOKING_READY = (
    ...
    "Анкета подождёт: дополните её и отправьте на проверку, когда захотите попасть в каталог.\n\n"
    "Откройте <b>«Обзор»</b> ниже."
)
TRAINER_AFTER_LINK_STEP_INVITE_SUBMIT = (
    "<b>Сейчас</b>: анкета почти готова — откройте <b>«Обзор»</b>, зайдите в <b>«Профиль»</b> в мини-приложении "
    "и сохраните анкету: заявка на проверку уйдёт автоматически. Обычно отвечаем в течение рабочего дня."
)
TRAINER_AFTER_LINK_STEP_PENDING = (
    "<b>Сейчас</b>: анкета на проверке у команды. Ничего нажимать не нужно — когда одобрят, "
    "в «Обзор» откроются все разделы. Статус смотрите в профиле в приложении."
)
TRAINER_AFTER_LINK_STEP_NEEDS_EDIT = (
    "<b>Сейчас</b>: модератор подсказал, что поправить.\n\n"
    "<b>Комментарий команды:</b>\n{feedback}\n\n"
    "Загляните в профиль в мини-приложении — и снова отправим на проверку."
)
...
TRAINER_EDITOR_OPEN_HINT = (
    "Открой <b>«Обзор»</b> слева от поля ввода и зайди в расписание — "
...
# Tone: «вы», тепло, без вины; без дублирования тревоги между /start, middleware и профилем в приложении.
TRAINER_GATE_CALLBACK_BLOCKED = (
    "Этот раздел пока закрыт. Откройте «Обзор» — там «Первые шаги», и всё станет на место."
)
TRAINER_GATE_BLOCKED_PROFILE = (
    "Расписание, заявки и записи откроются, как только в анкете появятся обязательные поля.\n\n"
    "Откройте «Обзор» → «Профиль» в мини-приложении — там видно, что ещё добавить."
)
TRAINER_GATE_BOOKING_READY = (
    "В <b>мини-приложении</b> уже можно открыть расписание и сделать первые записи — то же доступно и "
    "в этом чате (заметки, приглашения клиентов).\n\n"
    "Чтобы вас увидели в <b>общем каталоге</b>, дополните анкету и дождитесь одобрения — статус в «Профиле» в приложении.\n\n"
    "Откройте <b>«Обзор»</b> слева от поля ввода."
)
...
TRAINER_GATE_INVITE_SUBMIT = (
    "Анкета заполнена — осталось отправить её на проверку. Откройте «Обзор» → «Профиль» и в мини-приложении "
    "нажмите «Сохранить»: заявка уйдёт модератору автоматически.\n\n"
    "Обычно отвечаем в течение рабочего дня. Статус смотрите в профиле в приложении."
)
# Анкета отправлена, ждём модератора (без комментария «нужны правки»).
TRAINER_GATE_PENDING_MODERATION = (
    "Анкета на проверке. Ничего делать не нужно — мы напишем, как только будет решение. "
    "Когда одобрят, разделы откроются сами. Статус смотрите в профиле в приложении."
)
# Комментарий модератора при «нужны правки» (moderation_feedback).
TRAINER_GATE_NEEDS_EDIT = (
    "Модератор подсказал, что поправить.\n\n"
    "<b>Комментарий:</b>\n{feedback}\n\n"
    "Загляните в профиль в мини-приложении или в кабинете на сайте — и снова отправим на проверку."
)
