...
# Одна точка входа после invite/bind: каталог → карточка тренера и запись (не дублируем «Главная» / «Мои записи»).
CLIENT_BUTTON_TRAINER_AND_BOOKING = "Тренер и запись"
...
CLIENT_WELCOME_BIND_FIRST_IMPRESSION = (
    ...
    "<b>Ice Pro</b> — платформа для спорта и тренировок: мы соединяем тренеров и клиентов и поддерживаем развитие индустрии — это не только учёт записей.\n\n"
    "Кнопка ниже — к вашему тренеру и записи."
)
