...
- Trainer bot: «Вы», коротко и по делу; пуши: заголовок + суть + CTA «Обзор» (кнопка слева от поля ввода).
...
TRAINER_RELAY_REPLY_PROMPT = "Напишите здесь — отправим клиенту."
...
CLIENT_UNIVERSAL_INVITE_RETRY_LATER = (
    "👋 Спасибо, что перешли по ссылке тренера! Сейчас регистрация временно недоступна — "
    "попробуйте открыть эту же ссылку ещё раз через пару минут."
)
...
TRAINER_REQUEST_REMIND_HAS_SLOTS = (
    ...
    "По заявке есть свободные слоты — запишите клиента или напишите ему."
)
...
TRAINER_RESPOND_SUCCESS = "Отклик отправлен. Клиент увидит вас в списке и сможет записаться или написать."
...
TRAINER_DIGEST_OWED_LINE_CONFIRMS_ONLY = (
    "💬 Ждут вашего решения: <b>{count}</b> {word} на подтверждение."
)
...
TRAINER_DIGEST_MORNING_REC_FIRST_TIMER_OPENS_DAY = (
    "👉 Начните день с {name} — у клиента это первое занятие. "
...
TRAINER_DIGEST_MORNING_LITE_REC = (
    "👉 Ответ в первый час обычно решает, выберет клиент вас или другого тренера."
)
...
TRAINER_DIGEST_WEEKLY_REC_ALL_GOOD = (
    "👉 Неделя собрана. Воскресенье — ваше."
)
...
TRAINER_DIGEST_DROUGHT_OPEN_REQUESTS = (
    "💬 На неделе тихо — но вас ждут <b>{count}</b> {word} в каталоге. "
...
TRAINER_DIGEST_DROUGHT_DORMANT = (
    "💭 На неделе пусто. {names} давно не появлялись — возможно, стоит написать им?"
)
...
# Trainer: facts structured (date, time, clients), one action (nothing urgent). Client: same style.
CARE_PULSE_TRAINER_TOMORROW = (
    "📅 <b>Завтра у вас {count} {word}</b>\n\n"
    "Первая — в <b>{first_time}</b> · <b>{first_name}</b>\n\n"
    "Если произойдут изменения, сразу сообщим."
)
CARE_PULSE_TRAINER_OPEN_SLOTS = (
    "📅 <b>Есть свободные окна</b>\n\n"
    "В ближайшие 7 дней — <b>{count}</b> {word}.\n\n"
    "Клиенты уже видят их в Telegram и могут записаться самостоятельно."
)
CARE_PULSE_TRAINER_INVITE_BACK = (
    "👤 <b>{name} давно не был на тренировке</b>\n\n"
    "<b>{days}</b> {word} без занятий.\n\n"
    "Возможно, самое время напомнить о себе 🙂"
)
CARE_PULSE_TRAINER_QUIET = (
    "🌿 <b>Сегодня свободный день</b>\n\n"
    "На сегодня записей нет.\n\n"
    "Если появится новая тренировка — сообщим."
)
CARE_PULSE_CLIENT_CONFIRMED = (
    "✅ <b>Запись подтверждена</b>\n\n"
    "📅 <b>{when}, {day_name}</b>\n"
    "🕐 <b>{time}</b> · {duration} минут\n"
    "👤 <b>{trainer_name}</b>\n"
    "{arena_line}"
    "Напомним о тренировке за день.\n"
    "Если планы изменятся — свяжитесь с тренером."
)
CARE_PULSE_CLIENT_INVITE_BACK = (
    "👋 <b>Как насчёт следующей тренировки?</b>\n\n"
    "Вы уже занимались с <b>{trainer_name}</b> — можно выбрать удобное время для следующего занятия.\n\n"
    "Свободные слоты ждут в каталоге."
)
...
TRAINER_REQUEST_NOTIFICATION = (
    ...
    "Дальше: «Обзор» → <b>«Заявки»</b> — отклик и действия под заявкой."
)
...
CLIENT_WELCOME_BIND_FIRST_IMPRESSION = (
    ...
    "<b>Glide</b> — платформа для спорта и тренировок: мы соединяем тренеров и клиентов и поддерживаем развитие индустрии.\n\n"
...
CLIENT_PASS_WELCOME = (
    "👋 <b>{name} пригласил вас в Glide.</b>\n\n"
    "⛸ Мы развиваем сервис вместе с практикующими тренерами на льду — "
    "хоккей, катание, работа над техникой и ОФП.\n\n"
...
# в режим оценки продукта, а нам нужен режим работы.
TRAINER_AFTER_LINK_HERO = (
    "Чтобы записи не терялись в переписке.\n\n"
    "За две минуты у вас будет ссылка, по которой ученик сам выберет время. "
...
TRAINER_LINK_INVALID = "Ссылка недействительна или уже использована. Получите новую на сайте после оплаты."
TRAINER_LINK_TELEGRAM_CONFLICT = (
    "Этот Telegram уже привязан к <b>другому</b> профилю тренера в системе.\n\n"
    "Если вы уже работаете в боте — просто откройте чат и нажмите «Обзор» (команда /start без ссылки). "
    "Если это ошибка — напишите в поддержку (кнопка ниже)."
)
...
TRAINER_WELCOME_TRIAL_ACTIVATED = (
    ...
    "До <b>{expires_date}</b> у вас открыт полный доступ ко всем модулям платформы: "
    "CRM, онлайн-запись, абонементы, сертификаты, аналитика и группы.\n\n"
    "Когда пробный период закончится, собери свой план в разделе "
    "<b>«Подписка»</b> в «Обзор» — оставьте только то, что действительно используете."
)
...
TRAINER_FALLBACK = "Откройте «Обзор» слева от поля ввода — там все разделы. Помощь: /guide"
...
TRAINER_ERROR_RESPOND_FAILED = "Не удалось откликнуться (заявка уже закрыта или отклик уже был). Обновите «Заявки клиентов»."
TRAINER_ERROR_NO_SERVICES = (
    "У вас не указана ни одна услуга. Добавьте услугу в профиле в мини-приложении или в личном кабинете на сайте."
)
...
TRAINER_REQUEST_BOOK_NO_SLOTS_TWO_WEEKS = (
    "Нет свободных слотов на ближайшие 2 недели. Добавьте слоты в разделе «Расписание»."
)
TRAINER_ERROR_REQUEST_BOOK_PAYLOAD = (
    "Не удалось обработать выбор. Откройте список заявок и попробуйте снова."
)
TRAINER_ERROR_REQUEST_BOOK_PAYLOAD_SHORT = "Не удалось обработать выбор. Попробуйте снова."
TRAINER_ERROR_SLOT_TAKEN_FOR_REQUEST = (
    "Слот уже занят или недоступен. Выберите другой слот из списка."
)

# Trainer: Web App intros (HTTPS) — informal «ты» like TRAINER_START_WELCOME
TRAINER_EDITOR_OPEN_HINT = (
    "Откройте <b>«Обзор»</b> слева от поля ввода и перейдите в расписание — "
...
TRAINER_BOOKINGS_OPEN_WEBAPP = (
    "Откройте <b>«Моё расписание»</b> — занятые слоты с деталями записи: подтвердить, отменить, провести занятие, написать клиенту."
)
...
TRAINER_CLIENTS_OPEN_WEBAPP = (
    "Откройте список клиентов с записями: там видны телефон и последнее занятие."
)
...
TRAINER_PASSES_INTRO_WEBAPP = (
    "Настройте абонементы (N занятий за цену) и сертификаты (номинал на сумму или «любая сумма»). "
...
TRAINER_INVITE_INTRO_HTML = (
    "📣 <b>Пригласить ученика</b>\n\n"
    "Перешлите ему следующее сообщение."
)
# Сообщение, которое тренер пересылает ученику. Одна строка и одна ссылка.
# Раньше здесь было две ссылки (бот + каталог) и инструкция «меню слева → Тренеры и запись» —
# три объекта внимания у человека, который просто хочет записаться. Каталог ученику на этом
# шаге не нужен: он пришёл к конкретному тренеру, и ссылка ведёт прямо к его расписанию.
TRAINER_INVITE_PLAIN_CLIENT = (
    "Привет! Если будет удобно — можно заглянуть в расписание и записаться, это пара секунд\n"
    "{deep_link}"
)
# Совместимость: оба старых имени указывают на единственный текст.
TRAINER_INVITE_PLAIN_CLIENT_NO_CATALOG = TRAINER_INVITE_PLAIN_CLIENT
TRAINER_INVITE_PLAIN_CLIENT_WITH_CATALOG = TRAINER_INVITE_PLAIN_CLIENT
...
TRAINER_INVITE_ERR_PROFILE_INCOMPLETE = (
    ...
    "Укажите в профиле в мини-приложении или в кабинете на сайте и снова нажмите «Пригласить клиента»."
)
...
TRAINER_CANCEL_IDLE = "Нечего отменять. Профиль — в «Обзор»."
...
TRAINER_SCHEDULE_EMPTY = "Шаблон пуст. Ниже добавьте слоты в шаблон или сразу на выбранную неделю."
...
TRAINER_SCHEDULE_CHOOSE_WEEK = "На какую неделю добавить слоты?"
TRAINER_SCHEDULE_THIS_WEEK = "Эта неделя ({start}–{end})"
TRAINER_SCHEDULE_NEXT_WEEK = "Следующая неделя ({start}–{end})"
TRAINER_SCHEDULE_CHOOSE_DAY = "Выберите день недели:"
TRAINER_SCHEDULE_CHOOSE_TIME = "Отметьте все нужные часы (повторное нажатие снимает отметку), затем «Готово»:"
...
TRAINER_SCHEDULE_SELECT_AT_LEAST_ONE = "Выберите хотя бы один час."
...
TRAINER_SCHEDULE_TEMPLATE_CHOOSE_ANOTHER_DAY = "Выберите другой день или вернитесь в расписание."
...
TRAINER_CLIENT_INVITE_LINK_FOR_TRAINER = (
    "📣 <b>Пригласительная ссылка</b> · {client_name}\n\n"
...
# Voice (hard): warmth, play, respect. «вы». No guilt, no «заполни / добить / срочно», no alarms.
# Never-started (empty_form) is a warm invite into the cabinet, not a homework list.
# Photo: playful, Sanya-style («Улыбочкууу, сейчас вылетит птичка») then the point.
TRAINER_ONBOARDING_NUDGE_INTRO_D1 = "👋 <b>Кабинет уже почти ваш.</b>\n\n"
TRAINER_ONBOARDING_NUDGE_INTRO_D3 = "✨ <b>Остался маленький штрих.</b>\n\n"
TRAINER_ONBOARDING_NUDGE_INTRO_D7 = "🌟 <b>Вы совсем близко.</b>\n\n"

# Warming intros for trainers who opened Telegram but never started the wizard.
TRAINER_ONBOARDING_EMPTY_INTRO_D1 = "👋 <b>Рады, что вы заглянули.</b>\n\n"
TRAINER_ONBOARDING_EMPTY_INTRO_D3 = "✨ <b>Ваш кабинет уже ждёт.</b>\n\n"
TRAINER_ONBOARDING_EMPTY_INTRO_D7 = "🐧 <b>Место в каталоге пока за вами.</b>\n\n"

TRAINER_ONBOARDING_STAGE_EMPTY_FORM = (
    "За пару минут здесь появится живое расписание — слоты, записи и напоминания ученикам. "
    "Можно даже сделать пробную запись, просто чтобы увидеть, как всё работает."
)
TRAINER_ONBOARDING_STAGE_MISSING_FIELD = "Осталось совсем чуть-чуть: <b>{missing}</b>."
# Playful variant for the single most common case — только фото профиля не хватает.
TRAINER_ONBOARDING_STAGE_MISSING_PHOTO_ONLY = (
    "📸 Улыбочкууу, сейчас вылетит птичка! Добавьте фото профиля — "
    "ученики сразу вас узнают, и анкета выглядит живой."
)
TRAINER_ONBOARDING_STAGE_REJECTED_RESUBMIT = (
    "Модератор оставил замечания — это быстро поправить в профиле, и снова отправим на проверку."
)
TRAINER_ONBOARDING_STAGE_NOT_SUBMITTED = (
    "Анкета готова — осталось отправить её на проверку. Обычно отвечаем в течение рабочего дня."
)
TRAINER_ONBOARDING_STAGE_NO_BOOKING = (
    "Профиль уже живой. Сделайте пробную запись — так сразу видно, как выглядят слоты и напоминания."
)

TRAINER_ONBOARDING_TRIAL_DAYS_LEFT = (
    "\n\nПробный период ещё {days} дн. — спокойно хватит, чтобы всё настроить."
)
TRAINER_ONBOARDING_TRIAL_LAST_DAY = (
    "\n\nПробный период заканчивается сегодня — без спешки, просто чтобы вы знали."
)
...
TRAINER_PROFILE_ENRICH_NUDGE_P1 = (
    "🃏 <b>Цену занятия ученик пока не видит</b> :)\n\n"
    "Первая запись уже есть — без прайса всё сработало.\n\n"
    "Когда будет минута, в профиле можно поставить тариф "
    "и написать, что не входит в стоимость. Можно не сейчас."
)
TRAINER_PROFILE_ENRICH_NUDGE_P8 = (
    "🃏 Если в профиле ещё нет цены — это нормально.\n\n"
    "Один раз указать тариф и что не входит в стоимость — "
    "и не придётся объяснять в каждом чате. Запись от этого не зависит."
)
TRAINER_PROFILE_ENRICH_NUDGE_P21 = (
...
TRAINER_SUBSCRIPTION_GRANTED_BY_ADMIN = (
    ...
    "Тариф: <b>«{label}»</b>\n"
    "Действует до <b>{expires_date}</b>.\n\n"
    "Все включённые модули уже доступны в «Обзор»."
)
...
TRAINER_COLLECTIVE_INVITE_SEATS_FULL = "В студии заняты все места — попросите владельца расширить команду."
