---
name: "source-command-run-local"
description: "Migrated source command `run-local`"
---

# source-command-run-local

Use this skill when the user asks to run the migrated source command `run-local`.
...
6. **Запуск процессов — предпочти `nohup` в логи**, иначе фоновые вызовы из агента Codex часто **убивают дочерние процессы** после выхода оболочки:
...
В Codex: **Tasks: Run Task** → **`CRM: Tunnel → then all servers`** (см. `.vscode/tasks.json`).
